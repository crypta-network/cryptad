...
public abstract class RandomAccessBufferTestBase {
    ...
    private void innerTestWriteOverLimit(long sz, int choppedBytes) throws IOException {
        ...
        if (sz != 0 && choppedBytes < sz) {
            if (startAt >= 0) {
                readWriteMustSucceed(raf, startAt, buf, 0,
                                     buf.length); // Read, write up to the end work.
            } else {
                try {
                    readWriteMustSucceed(raf, startAt, buf, 0,
                                         buf.length); // Read, write up to the end work.
        ...
        if (startAt + 1 >= 0) {
            ...
            try {
                readWriteMustSucceed(raf, startAt + 1, buf, 0,
                                     buf.length); // Read, write up to the end work.
                fail("Should fail to read at negative index");
            } catch (IllegalArgumentException e) {
                // Ok.
    ...
    private void readWriteMustSucceed(
        RandomAccessBuffer raf, long startAt, byte[] buf, int offset, int length) throws IOException {
        raf.pread(startAt, buf, 0, buf.length); // Should work
        raf.pwrite(startAt, buf, 0, buf.length); // Should work
    }

    private void readWriteMustFail(
        RandomAccessBuffer raf, long startAt, byte[] buf, int offset, int length) throws IOException {
        if (length == 0) {
            return; // NOP.
        }
