...
public class HTMLEncoderDecoderTest {
  ...
  /** Tests decode(String) method trying to decode entity by entity */
  @Test
  public void testDecodeSingleEntities() {
    for (int i = 0; i < UTFUtil.HTML_ENTITIES_UTF.length; i++) {
      assertEquals(
          HTMLDecoder.decode(UTFUtil.HTML_ENTITIES_UTF[i][1]), UTFUtil.HTML_ENTITIES_UTF[i][0]);
    }
  }

  /**
   * Tests decode(String) method trying to decode a long String with all possible entity appended
   */
  @Test
  ...
  /**
   * Tests decode(String) method trying to decode incomplete entities. The incomplete entity must
   * remain the same as before encoding
   */
  @Test
  ...
  /**
   * Tests compact(String) method trying to compact String with repeated whitespaces of every kind
  ...
  /** Tests isWhiteSpace() method against all possible HTML white space type */
  @Test
