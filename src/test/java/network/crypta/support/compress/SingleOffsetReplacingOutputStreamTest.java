...
import network.crypta.support.compress.SingleOffsetReplacingOutputStream;
...
public class SingleOffsetReplacingOutputStreamTest {
    ...
    private void filterOutputStreamWithOffsetAndReplacementWritingSingleBytes(
        ...
        filterOutputStreamWithOffsetAndReplacement(blockSize, numberOfBlocks, replacementOffset,
                                                   (buffer, length, repetitions, outputStream) -> {
                                                       for (int index = 0;
                                                            index < length * repetitions; index++) {
        ...
        filterOutputStreamWithOffsetAndReplacement(blockSize, numberOfBlocks, replacementOffset,
                                                   (buffer, length, repetitions, outputStream) -> {
                                                       for (int blockIndex = 0;
                                                            blockIndex < repetitions; blockIndex++) {
        ...
        filterOutputStreamWithOffsetAndReplacement(blockSize, numberOfBlocks, replacementOffset,
                                                   ((buffer, length, repetitions, outputStream) -> {
                                                       for (int blockIndex = 0;
                                                           ...
                                                           outputStream.write(buffer, length / 2,
                                                                              length - (length / 2));
                                                       }
                                                   }));
    }
    ...
    private void filterOutputStreamWithOffsetAndReplacement(
        int blockSize, int numberOfBlocks, int replacementOffset,
        BufferToOutputStreamCopierStrategy bufferToOutputStreamCopierStrategy) throws IOException {
        ...
        SingleOffsetReplacingOutputStream outputStream =
            new SingleOffsetReplacingOutputStream(byteArrayOutputStream, replacementOffset,
                                                  (byte) (bufferToWrite[replacementOffset % blockSize] ^
                                                          0xff));
        ...
        for (int offset = 0; offset < blockSize * numberOfBlocks; offset++) {
            assertThat("offset " + offset, writtenBuffer[offset], equalTo(
                (byte) (bufferToWrite[offset % blockSize] ^
