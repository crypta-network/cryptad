...
public class CSSParserTest {
  ...
  static {
    ...
    CSS2_SELECTOR.put(
        "span[class=example] { color: blue; }", "span[class=example] { color: blue; }");
    CSS2_SELECTOR.put(
        "span[hello=\"Cleveland\"][goodbye=\"Columbus\"] { color: blue; }",
        "span[hello=\"Cleveland\"][goodbye=\"Columbus\"] { color: blue; }");
    CSS2_SELECTOR.put(
        "div > p:first-child { text-indent: 0 }", "div>p:first-child { text-indent: 0 }");
    CSS2_SELECTOR.put(
        "div > p:FIRST-CHILD { text-indent: 0 }", "div>p:FIRST-CHILD { text-indent: 0 }");
    CSS2_SELECTOR.put(
    ...
    // If there was scripting it would not be safe, although datastore probing is probably the
    // greater threat.
    ...
    CSS2_SELECTOR.put(
        "p:first-line { text-transform: uppercase;}", "p:first-line { text-transform: uppercase;}");
    // CONFORMANCE: :first-line can only be attached to block-level, we don't enforce this, it is
    // not dangerous.
    CSS2_SELECTOR.put(
    ...
    CSS2_SELECTOR.put(
        "h1[foo] h2 > p + b:before { color: green;}", "h1[foo] h2>p+b:before { color: green;}");
    CSS2_SELECTOR.put(
        "table          { border-collapse: collapse; border: 5px solid yellow; } *#col1         {"
            + " border: 3px solid black; } td             { border: 1px solid red; padding: 1em; }"
            + " td.cell5       { border: 5px dashed blue; } td.cell6       { border: 5px solid"
            + " green; }",
        "table { border-collapse: collapse; border: 5px solid yellow; } *#col1 { border: 3px solid"
            + " black; } td { border: 1px solid red; padding: 1em; } td.cell5 { border: 5px dashed"
            + " blue; } td.cell6 { border: 5px solid green; }");
    CSS2_SELECTOR.put(
        "td { border-right: hidden; border-bottom: hidden }",
        "td { border-right: hidden; border-bottom: hidden }");

    // CONFORMANCE: We combine pseudo-classes and pseudo-elements, so we allow pseudo-elements on
    // earlier selectors. This is against the spec, CSS2 section 5.10.
  }

  private static final HashSet<String> CSS2_BAD_SELECTOR = new HashSet<>();
  ...
  private static final HashMap<String, String> CSS3_SELECTOR = new HashMap<>();
  ...
  private static final HashSet<String> CSS3_BAD_SELECTOR = new HashSet<>();
  ...
  private static final HashMap<String, String> CSS_SELECTOR_LEVEL4 = new HashMap<>();
  ...
  private static final HashSet<String> CSS_BAD_SELECTOR_LEVEL4 = new HashSet<>();
  ...
  private static final String CSS_BACKGROUND_URL =
      "* { background:"
          + " url(/SSK@qd-hk0vHYg7YvK2BQsJMcUD5QSF0tDkgnnF6lnWUH0g,xTFOV9ddCQQk6vQ6G~jfL6IzRUgmfMcZJ6nuySu~NUc,AQACAAE/activelink-index-text-76/activelink.png);"
          + " }";
  ...
  private static final String CSS_IMPORT2 =
      "@import"
          + " url(\"/chk@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/1-1.html\")"
          + " screen;";
  private static final String CSS_IMPORT2C =
      "@import"
          + " url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/1-1.html?type=text/css&maybecharset=UTF-8\")"
          + " screen;";
  ...
  private static final String CSS_IMPORT_SPACE_IN_STRINGC =
      "@import"
          + " url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page?type=text/css&maybecharset=UTF-8\")"
          + " screen;";
  ...
  private static final String CSS_IMPORT_QUOTED_STUFFC =
      "@import"
          + " url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page%20%29%20%5c%20%27%20%22%20?type=text/css&maybecharset=UTF-8\")"
          + " screen;";
  ...
  private static final String CSS_IMPORT_QUOTED_STUFF2C =
      "@import"
          + " url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page%20%29%20%5c%20%27%20%22?type=text/css&maybecharset=UTF-8\")"
          + " screen;";

  private static final String CSS_IMPORT_NOURL_TWOMEDIAS =
      "@import"
          + " \"/chk@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/1-1.html\""
          + " screen tty;";
  ...
  static {
    ...
    // Invalid combination
    propertyTests.put(
        "@media speech { h1 { azimuth: left-side behind 30deg }; }", "@media speech { h1 { }}");
    propertyTests.put(
        "@media speech { h1 { azimuth: inherit }; }", "@media speech { h1 { azimuth: inherit }}");
    // Wrong media type
    ...
    propertyTests.put(
        "td { background-color: rgb(255, 255, 255)}", "td { background-color: rgb(255, 255, 255)}");
    ...
    propertyTests.put(
        "td { background-color: rgb(100%,0%,50%)}", "td { background-color: rgb(100%,0%,50%)}");
    propertyTests.put(
        "td { background-color: rgb(100%, 0%, 50%)}", "td { background-color: rgb(100%, 0%, 50%)}");
    // Values outside the standard RGB device gamut are allowed by the spec, they will be clipped
    // and may be representible on some devices.
    propertyTests.put(
        "td { background-color: rgb(300, 0, 0)}", "td { background-color: rgb(300, 0, 0)}");
    propertyTests.put(
        "td { background-color: rgb(255, -10, 0)}", "td { background-color: rgb(255, -10, 0)}");
    propertyTests.put(
        "td { background-color: rgb(110%, 0%, 0%)}", "td { background-color: rgb(110%, 0%, 0%)}");
    propertyTests.put(
    ...
    // Fractional lengths
    propertyTests.put(
        "h3 { background-position: 3.3cm 20%}", "h3 { background-position: 3.3cm 20%}");
    // Negative fractional lengths
    propertyTests.put(
        "h3 { background-position: -0.87em 20%}", "h3 { background-position: -0.87em 20%}");
    ...
    propertyTests.put(
        ...
        "h3 { background-image: url(\"/KSK@something.png\");}");
    propertyTests.put(
        "h3 { background-image: url(/KSK@\\something.png?type=image/png\\26 force=true);}",
    ...
    propertyTests.put(
        "h3 { background-image: url(/KSK@\\));}", "h3 { background-image: url(\"/KSK@%29\");}");
    propertyTests.put(
        "h3 { background-image: url(/KSK@\\');}", "h3 { background-image: url(\"/KSK@%27\");}");
    propertyTests.put(
        "h3 { background-image: url(/KSK@\\\");}", "h3 { background-image: url(\"/KSK@%22\");}");
    propertyTests.put(
        "h3 { background-image: url(/KSK@\\ );}", "h3 { background-image: url(\"/KSK@%20\");}");
    ...
    propertyTests.put(
        ...
        "h3 { background: scroll #f00"
            ...
            + " }");
    propertyTests.put(
        "h3 { background: scroll rgb(100%, 2%, 1%)"
            + " url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page\")"
            + " }",
        "h3 { background: scroll rgb(100%, 2%, 1%)"
            + " url(\"/CHK@~~vxVQDfC9m8sR~M9zWJQKzCxLeZRWy6T1pWLM2XX74,2LY7xwOdUGv0AeJ2WKRXZG6NmiUL~oqVLKnh3XdviZU,AAIC--8/test%20page\")"
    ...
    propertyTests.put(
        ...
        "table { counter-increment: \\202 ounter1 1}");
    propertyTests.put(
        "table { counter-increment: c\\ounter1 1}", "table { counter-increment: c\\ounter1 1}");
    propertyTests.put(
        "table { counter-increment: c\\ ounter1 1}", "table { counter-increment: c\\ ounter1 1}");
    propertyTests.put(
    ...
    // Content tests
    propertyTests.put(
        "h1 { content: \"string with spaces\" }", "h1 { content: \"string with spaces\" }");
    propertyTests.put(
    ...
    propertyTests.put(
        "h1 { content: attr(\\000202\\ test);}", "h1 { content: attr(\\000202\\ test);}");
    propertyTests.put("h1 { content: attr(\\;\\ test);}", "h1 { content: attr(\\;\\ test);}");
    propertyTests.put("h1 { content: attr(\\}\\ test);}", "h1 { content: attr(\\}\\ test);}");
    // Refer to counters
    propertyTests.put(
        "h1 { content: counter(\\202 \\ \\test, none);}",
    ...
    propertyTests.put(
        "h2 { font-family: Verdana,sans-serif }", "h2 { font-family: Verdana,sans-serif }");
    ...
    // Shape
    propertyTests.put(
        "p { clip: rect(5px, 40px, 45px, 5px); }", "p { clip: rect(5px, 40px, 45px, 5px); }");
    propertyTests.put(
        "p { clip: rect(auto, auto, 45px, 5px); }", "p { clip: rect(auto, auto, 45px, 5px); }");
    ...
    propertyTests.put(
        "* { causta: \"}\" + ({7} * '\\'') } h2 { color: red;}", "* { } h2 { color: red;}");
    propertyTests.put(
        "* { causta: \"}\" + ({inner-property: blahblahblah;} * '\\'') } h2 { color: red;}",
        "* { } h2 { color: red;}");

    // Auto-close of style sheet. NOT IMPLEMENTED! The first test tests that we handle unclosed
    ...
    // Box model
    propertyTests.put(
        "LI { color: white; background: blue; margin: 12px 12px 12px 12px; padding: 12px 0px 12px"
    ...
    propertyTests.put(
        ...
        "#xy34 { border-style: none dotted solid inset }");
    propertyTests.put(
        "#xy34 { border-style: solid dotted }", "#xy34 { border-style: solid dotted }");
    propertyTests.put(
        "h1 { border-bottom: thick solid red }", "h1 { border-bottom: thick solid red }");
    ...
    propertyTests.put(
        "div { display: list-item block flow; }", "div { display: list-item block flow; }");
    propertyTests.put(
        "div { display: block list-item flow; }", "div { display: block list-item flow; }");
    ...
    propertyTests.put(
        "div { display: ruby-text-container; }", "div { display: ruby-text-container; }");
    propertyTests.put(
    ...
    propertyTests.put(
        ...
        "body { top: auto; left: inherit; right: 23em; bottom: 3.2% }");
    propertyTests.put(
        "EM { padding: 2px; margin: 1em; border-width: medium; border-style: dashed; line-height:"
            + " 2.4em; }",
        "EM { padding: 2px; margin: 1em; border-width: medium; border-style: dashed; line-height:"
            + " 2.4em; }");
    propertyTests.put(
        ...
        "p { width: 10em; border: solid aqua; }\n"
            + "span { float: left; width: 5em; height: 5em; border: solid blue; }");
    propertyTests.put(
        "img.icon { float: left; margin-left: 0; }", "img.icon { float: left; margin-left: 0; }");
    propertyTests.put(
        "i { float: inherit } b { float: none }", "i { float: inherit } b { float: none }");
    ...
    propertyTests.put(
        ...
        "blockquote { direction: rtl; unicode-bidi: BIDI-OVERRIDE }");
    propertyTests.put(
        "div { border-inline-start-color: transparent; border-inline-start-style: dotted;"
    ...
    propertyTests.put(
        ...
        "p { height: 100px; min-height: inherit; max-height: 33% }");
    propertyTests.put(
        "div { line-height: 1.2; font-size: 10pt }", "div { line-height: 1.2; font-size: 10pt }");
    propertyTests.put(
        "div { line-height: 1.2em; font-size: 10pt }",
        "div { line-height: 1.2em; font-size: 10pt }");
    propertyTests.put(
        "div { line-height: 120%; font-size: 10pt }", "div { line-height: 120%; font-size: 10pt }");
    propertyTests.put(
    ...
    propertyTests.put(
        ...
        "body:after { content: \"The End\"; display: block; margin-top: 2em; text-align: center;"
            + " }");
    propertyTests.put(
        "h1:before { content: \"Chapter \" counter(test) \" \" open-quote attr(chaptername)"
            + " close-quote }",
        "h1:before { content: \"Chapter \" counter(test) \" \" open-quote attr(chaptername)"
            + " close-quote }");
    propertyTests.put(
    ...
    propertyTests.put(
        "blockquote p:before     { content: open-quote } blockquote p:after      { content:"
            + " no-close-quote } blockquote p.last:after { content: close-quote }",
    ...
    propertyTests.put(
        ...
        "BLOCKQUOTE:after { content: \" [\" counter(bq, lower-greek) \"]\" }");
    propertyTests.put(
        "ol { list-style-type: lower-roman }   ", "ol { list-style-type: lower-roman }   ");
    propertyTests.put(
    ...
    propertyTests.put(
        ...
        "ul { list-style: upper-roman inside url(\"ul.png\") }");
    propertyTests.put(
        "ol.alpha > li { list-style: lower-alpha }", "ol.alpha>li { list-style: lower-alpha }");
    ...
    propertyTests.put(
        ...
        "@page :first { margin-top: 10cm } * { margin: 10px }");
    propertyTests.put(
        "h1 { page-break-before: always; orphans: 3; widows: 4 } h2 { page-break-after: inherit;"
            + " orphans: 277; widows: inherit } h3 { page-break-inside: avoid; orphans: inherit;"
            + " widows: 10 }",
        "h1 { page-break-before: always; orphans: 3; widows: 4 } h2 { page-break-after: inherit;"
            + " orphans: 277; widows: inherit } h3 { page-break-inside: avoid; orphans: inherit;"
            + " widows: 10 }");
    ...
    propertyTests.put(
        ...
        "body { background: url(\"background.jpeg\") }");
    propertyTests.put(
        "body { background-color: green } table { background-color: transparent } h1, h2, h3 {"
    ...
    propertyTests.put(
        ...
        "body { background: white url(\"ledger.png\") fixed; }");
    propertyTests.put(
        "body { background-position: 100% 100% } h1 { background-position: 3% bottom } h2 {"
            + " background-position: 3cm 2mm } h3 { background-position: right } h4 {"
            + " background-position: center bottom } h5 { background-position: inherit }",
        "body { background-position: 100% 100% } h1 { background-position: 3% bottom } h2 {"
            + " background-position: 3cm 2mm } h3 { background-position: right } h4 {"
            + " background-position: center bottom } h5 { background-position: inherit }");
    propertyTests.put(
    ...
    propertyTests.put(
        "div.important { text-align: center }", "div.important { text-align: center }");
    propertyTests.put("a:visited,a:link { text-decoration: underline }", "");
    propertyTests.put(
        "a:any-link { text-decoration: underline }", "a:any-link { text-decoration: underline }");
    propertyTests.put(
        "a:any-link { text-decoration: underline red }",
        "a:any-link { text-decoration: underline red }");
    propertyTests.put(
        "blockquote { text-decoration: underline overline line-through blink } h1 {"
            + " text-decoration: none } h2 { text-decoration: inherit }",
        "blockquote { text-decoration: underline overline line-through blink } h1 {"
            + " text-decoration: none } h2 { text-decoration: inherit }");
    propertyTests.put(
        "blockquote { letter-spacing: 0.1em }", "blockquote { letter-spacing: 0.1em }");
    propertyTests.put(
        "blockquote { letter-spacing: normal }", "blockquote { letter-spacing: normal }");
    ...
    propertyTests.put(
        "pre        { white-space: pre } p          { white-space: normal } td[nowrap] {"
            + " white-space: nowrap }",
        "pre { white-space: pre } p { white-space: normal } td[nowrap] { white-space: nowrap }");
    propertyTests.put(
        "pre[wrap]  { white-space: pre-wrap }", "pre[wrap] { white-space: pre-wrap }");
    propertyTests.put(
    ...
    propertyTests.put(
        ...
        "table { table-layout: fixed; margin-left: 2em;margin-right: 2em }");
    propertyTests.put(
        "table { border-collapse: collapse; border-spacing: 12em 11cm; border-spacing: 10px;"
            + " border-spacing: 0; border-spacing: inherit }",
        "table { border-collapse: collapse; border-spacing: 12em 11cm; border-spacing: 10px;"
            + " border-spacing: 0; border-spacing: inherit }");
    propertyTests.put(
        "table      { border: outset 10pt; border-collapse: separate; border-spacing: 15pt } td {"
            + " border: inset 5pt } td.special { border: inset 10pt }",
        "table { border: outset 10pt; border-collapse: separate; border-spacing: 15pt } td {"
            + " border: inset 5pt } td.special { border: inset 10pt }");
    // Checking properties against elements is invalid because of inheritance. empty-cells applies
    // to td, th only, but if set on table it is inherited, this example taken from CSS2.1 spec.
    ...
    propertyTests.put(
        ":any-link { cursor: url(example.svg#linkcursor) 2, url(hyper.cur), pointer }",
        ":any-link { }");
    ...
    propertyTests.put(
        ":focus  { outline: thick solid black }", ":focus { outline: thick solid black }");
    propertyTests.put(
        ":active { outline: thick solid red }", ":active { outline: thick solid red }");
    ...
    propertyTests.put(
        ...
        "@media aural {\n  body { voice-family: Paul }}");
    propertyTests.put(
        "@media speech { h1, h2, h3, h4, h5, h6 { voice-family: Paul; stress: 20; richness: 90;"
            + " cue-before: url(\"ping.au\") } p.heidi { azimuth: center-left } p.peter { azimuth:"
            + " right } p.goat { volume: x-soft }}",
        "@media speech { h1, h2, h3, h4, h5, h6 { voice-family: Paul; stress: 20; richness: 90;"
            + " cue-before: url(\"ping.au\") } p.heidi { azimuth: center-left } p.peter { azimuth:"
            + " right } p.goat { volume: x-soft }}");
    propertyTests.put(
        "@media speech { body { volume: 10 } h1, h2 { volume: 50% } h3, h4 { volume: silent } p {"
            + " volume: inherit }}",
        "@media speech { body { volume: 10 } h1, h2 { volume: 50% } h3, h4 { volume: silent } p {"
            + " volume: inherit }}");
    propertyTests.put(
        "@media aural { q { speak: spell-out }}", "@media aural { q { speak: spell-out }}");
    propertyTests.put(
        "@media aural { blockquote { pause-after: 1s } h1 { pause-before: 10% } p { pause-after:"
            + " inherit }}",
        "@media aural { blockquote { pause-after: 1s } h1 { pause-before: 10% } p { pause-after:"
            + " inherit }}");
    propertyTests.put(
    ...
    propertyTests.put(
        ...
        "@media speech { span.quiet { play-during: none }}");
    propertyTests.put(
        "@media speech { h1   { elevation: above } tr.a { elevation: 60deg } tr.b { elevation:"
            + " 30deg } tr.c { elevation: level }}",
        "@media speech { h1 { elevation: above } tr.a { elevation: 60deg } tr.b { elevation: 30deg"
    ...
    propertyTests.put(
        ...
        "@media speech { body { pitch: x-low } i { pitch: 5kHz } b { pitch: 500Hz }}");
    propertyTests.put(
        "@media speech { body { pitch-range: 50; stress: 20 } h1,h2,h3 { pitch-range: 10.0; stress:"
            + " 5.0 } p { pitch-range: inherit; stress: inherit; richness: 20 }}",
        "@media speech { body { pitch-range: 50; stress: 20 } h1,h2,h3 { pitch-range: 10.0; stress:"
            + " 5.0 } p { pitch-range: inherit; stress: inherit; richness: 20 }}");
    propertyTests.put(
        ...
        "@media speech { .phone { speak-punctuation: code; speak-numeral: digits }}");
    propertyTests.put(
        "@media speech { table { speak-header: always } table.quick { speak-header: once }"
            + " table.sub { speak-header: inherit }}",
        "@media speech { table { speak-header: always } table.quick { speak-header: once }"
            + " table.sub { speak-header: inherit }}");
    propertyTests.put(
        "@media speech { h1 { voice-family: announcer, male } p.part.romeo  { voice-family: romeo,"
            + " male } p.part.juliet { voice-family: juliet, female }}",
        "@media speech { h1 { voice-family: announcer, male } p.part.romeo { voice-family: romeo,"
            + " male } p.part.juliet { voice-family: juliet, female }}");
    ...
    propertyTests.put(
        "div { flex-direction: column-reverse; }", "div { flex-direction: column-reverse; }");
    propertyTests.put(
        "div { order: 5; flex-basis: content; }", "div { order: 5; flex-basis: content; }");
    propertyTests.put(
        "div { justify-content: flex-start; }", "div { justify-content: flex-start; }");
    ...
    propertyTests.put(
        "div { justify-content: space-between; }", "div { justify-content: space-between; }");
    propertyTests.put(
        "div { justify-content: space-around; }", "div { justify-content: space-around; }");
    propertyTests.put(
        "div { justify-items: legacy center; }", "div { justify-items: legacy center; }");
    ...
    propertyTests.put(
        "div { align-content: space-between; }", "div { align-content: space-between; }");
    propertyTests.put(
        "div { align-content: true flex-start; }", "div { align-content: true flex-start; }");
    propertyTests.put(
        "div { align-content: flex-start safe; }", "div { align-content: flex-start safe; }");
    ...
    propertyTests.put(
        "button#foo { nav-left: #bar \"sidebar\"; }", "button#foo { nav-left: #bar \"sidebar\"; }");
    ...
    propertyTests.put(
        "div { transition-timing-function: ease; }", "div { transition-timing-function: ease; }");
    // valid, 2 values
    propertyTests.put(
        "div { transition-duration: 5s, 1s; }", "div { transition-duration: 5s, 1s; }");
    ...
    // writing mode
    propertyTests.put(
        "#a { writing-mode: vertical-rl; text-underline-position: left; }",
        "#a { writing-mode: vertical-rl; text-underline-position: left; }");
    propertyTests.put(
        "#b { writing-mode: horizontal-tb; text-underline-position: auto; inline-size: max-content;"
            + " block-size: 200px; }",
        "#b { writing-mode: horizontal-tb; text-underline-position: auto; inline-size: max-content;"
            + " block-size: 200px; }");
    ...
    propertyTests.put(
        "#foo { background: url(\"1.png\"); background-blend-mode: darken; }",
        "#foo { background: url(\"1.png\"); background-blend-mode: darken; }");
    propertyTests.put("#foo {mix-blend-mode: luminosity; }", "#foo {mix-blend-mode: luminosity; }");

    // new property values
    propertyTests.put(
        "div { overflow: clip; clear: inline-end; text-decoration: revert; float: inline-end;}",
        "div { overflow: clip; clear: inline-end; text-decoration: revert; float: inline-end;}");
    propertyTests.put("#a {unicode-bidi: isolate;}", "#a {unicode-bidi: isolate;}");
    propertyTests.put(
        "textarea#x {caret-color: currentcolor;}", "textarea#x {caret-color: currentcolor;}");
    propertyTests.put(
        "div { word-wrap: anywhere; overflow-wrap: anywhere; }",
        "div { word-wrap: anywhere; overflow-wrap: anywhere; }");
    propertyTests.put(
        "div { white-space-collapse: collapse; }", "div { white-space-collapse: collapse; }");
    propertyTests.put(
        "#a { word-break: keep-all; font-kerning: none; }",
        "#a { word-break: keep-all; font-kerning: none; }");
    ...
    propertyTests.put(
        "#x { list-style-type: korean-hanja-formal }",
        "#x { list-style-type: korean-hanja-formal }");
    propertyTests.put("#x { list-style-type: \"*\" }", "#x { list-style-type: \"*\" }");
    propertyTests.put(
        "#x { max-inline-size: none; min-block-size: auto; }",
        "#x { max-inline-size: none; min-block-size: auto; }");
    propertyTests.put("#x { text-combine-upright: all; }", "#x { text-combine-upright: all; }");
    propertyTests.put(
        "#x { text-decoration-thickness: 5px; }", "#x { text-decoration-thickness: 5px; }");
    propertyTests.put(
        "#x { dominant-baseline: alphabetic; }", "#x { dominant-baseline: alphabetic; }");
    propertyTests.put(
        "#x { margin-block: 3px 3%; margin-inline: 5%; margin-inline-end: 4px; margin-inline-start:"
            + " auto; }",
        "#x { margin-block: 3px 3%; margin-inline: 5%; margin-inline-end: 4px; margin-inline-start:"
            + " auto; }");
    propertyTests.put(
        "#x { padding-block: 3px 3%; padding-inline: 5%; padding-inline-end: 4px;"
            + " padding-inline-start: 1em; }",
        "#x { padding-block: 3px 3%; padding-inline: 5%; padding-inline-end: 4px;"
            + " padding-inline-start: 1em; }");
    propertyTests.put("#x { text-orientation: upright; }", "#x { text-orientation: upright; }");
    propertyTests.put(
        "#x { scroll-snap-align: start end; }", "#x { scroll-snap-align: start end; }");
    propertyTests.put("#x { scroll-snap-stop: always; }", "#x { scroll-snap-stop: always; }");
    propertyTests.put(
        "#x { scroll-snap-type: both proximity; } #y { scroll-snap-type: x; }",
        "#x { scroll-snap-type: both proximity; } #y { scroll-snap-type: x; }");
    propertyTests.put(
        "#box1:hover {rotate: 90deg;} #box2:hover {rotate: y 180deg;} #box3:hover {rotate: 1 2 1"
            + " 360deg;} #box4:hover {rotate: none;}",
        "#box1:hover {rotate: 90deg;} #box2:hover {rotate: y 180deg;} #box3:hover {rotate: 1 2 1"
            + " 360deg;} #box4:hover {rotate: none;}");
    propertyTests.put(
        "#a {rotate: 1;} #b {rotate: 0.1;} #c {rotate: 1 2 3 4;} #d {rotate: x;}",
        "#a {} #b {} #c {} #d {}");
    propertyTests.put(
        "#a {object-position: top;} #b {object-position: 25% 75%;} #c {object-position: 1cm top;}"
            + " #d {object-position: bottom 10px right 20px;}",
        "#a {object-position: top;} #b {object-position: 25% 75%;} #c {object-position: 1cm top;}"
            + " #d {object-position: bottom 10px right 20px;}");
    propertyTests.put(
        "#a {background-position: top;} #b {background-position: 25% 75%;} #c {background-position:"
            + " 1cm top;} #d {background-position: bottom 10px right 20px;}",
        "#a {background-position: top;} #b {background-position: 25% 75%;} #c {background-position:"
            + " 1cm top;} #d {background-position: bottom 10px right 20px;}");
    propertyTests.put(
        "#a {object-position: top top;} #b {object-position: 25% top 75% left;} #c"
            + " {object-position: left 10px right 20px;} #d {object-position: top right bottom"
            + " left;}",
        "#a {} #b {} #c {} #d {}");
    propertyTests.put(
        "td { border-inline-end: 2px dotted; border-block-start: medium dashed blue }",
        "td { border-inline-end: 2px dotted; border-block-start: medium dashed blue }");

    // text-emphasis
    propertyTests.put(
        "#x { text-emphasis: triangle blue; }",
        "#x { text-emphasis: triangle blue; }"); // java.lang.NullPointerException
    propertyTests.put(
        "#x { text-emphasis: filled triangle blue; }",
        "#x { text-emphasis: filled triangle blue; }");
    propertyTests.put(
        "#x { text-emphasis-style: triangle; text-emphasis-color: blue; }",
        "#x { text-emphasis-style: triangle; text-emphasis-color: blue; }");
    // text-shadow
    propertyTests.put(
        "#x { text-shadow: 1px 1px 2px black; }", "#x { text-shadow: 1px 1px 2px black; }");
    propertyTests.put(
        "#x { text-shadow: #fc0 1px 0 10px; }", "#x { text-shadow: #fc0 1px 0 10px; }");
    propertyTests.put(
        "#x { text-shadow: 5px 5px #558abb; }", "#x { text-shadow: 5px 5px #558abb; }");
  ...
  private void testBadSelectorFiltering(Set<String> badSelectorSet)
    ...
    for (String key : badSelectorSet) {
      assertEquals(
          "Bad selector filtering should produce empty string: '" + key + "'", "", filter(key));
    }
  }
  ...
  public void testImports() throws IOException, URISyntaxException {
    ...
    assertEquals(
        ...
        filter(CSS_IMPORT_NOURL_TWOMEDIAS));
    assertEquals(
        "key=" + CSS_IMPORT_UNQUOTED + " should be empty", "", filter(CSS_IMPORT_UNQUOTED));
    assertEquals(
  ...
  public void testCharset() throws IOException, URISyntaxException {
    ...
    assertThrows(
        "Bogus @charset should have been deleted", IOException.class, () -> filter(testFail));
  ...
  private void getCharsetTest(String charset, String family)
      throws IOException, URISyntaxException {
  ...
  private void charsetTestUnsupported(String charset) throws IOException, URISyntaxException {
    ...
    MatcherAssert.assertThat(
        "ContentFilter.filter() returned charset \""
    ...
    String filtered = new String(BucketTools.toByteArray(outputBucket), charset);
    assertEquals(
        "ContentFilter.filter() returns something: \""
            + filtered
            + "\" should be empty as unsupported charset, original: \""
            + original
            + "\" for charset \""
            + charset
            + "\"",
