...
public class JPEGFilterTest {
  ...
  public void testThatAThumbnailExtensionCodeIsPreserved() throws IOException {
    ...
    jpegFilter.readFilter(
        inputStream, outputStream, "UTF-8", new HashMap<>(), null, new NullFilterCallback());
  ...
  private byte[] createValidJpegFileWithThumbnail() throws IOException {
    ...
    writeAppMarker(
        jpegFile,
        new byte[] {
          0x4a, 0x46, 0x49, 0x46, 0x00, 0x01, 0x02, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
        });
    writeAppMarker(
        jpegFile, new byte[] {0x4a, 0x46, 0x58, 0x58, 0x00, 0x13, 0x01, 0x01, 0x00, 0x7f, 0x00});
  ...
  private void writeAppMarker(OutputStream outputStream, byte[] payload) throws IOException {
    ...
    outputStream.write(
        new byte[] {
          (byte) ((payloadLengthIncludingLength >> 8) & 0xff),
          (byte) (payloadLengthIncludingLength & 0xff)
        });
