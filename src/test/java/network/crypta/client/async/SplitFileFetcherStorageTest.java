...
public class SplitFileFetcherStorageTest {
  ...
  private static class TestSplitfile {
    private TestSplitfile(
        Bucket data,
        Metadata m,
        byte[][] originalDataBlocks,
        byte[][] originalCheckBlocks,
        ClientCHK[] dataKeys,
        ClientCHK[] checkKeys,
        byte[] cryptoKey,
    ...
    public SplitFileFetcherStorage createStorage(final StorageCallback cb, FetchContext ctx)
      ...
      return new SplitFileFetcherStorage(
          ...
          false,
          (short) COMPATIBILITY_MODE.ordinal(),
          ctx,
          false,
          salt,
          URI,
     ...
     *     create the splitfile. This must again be consistent with the rest, as it is sometimes
     *     used in decoding.
     * @return
     * @throws IOException
     * @throws CHKEncodeException
    ...
    static TestSplitfile constructMultipleSegments(
      ...
      Metadata m =
          new Metadata(
              ...
              cryptoAlgorithm,
              cryptoKey,
              true /* FIXME try older splitfiles pre-single-key?? */,
              0);
      // Make sure the metadata is reusable.
      // FIXME also necessary as the above constructor doesn't set segments.
      Bucket metaBucket = m.toBucket(bf);
