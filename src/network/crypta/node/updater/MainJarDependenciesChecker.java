...
/**
 * Parses the dependencies.properties file and ensures we have all the libraries required to use the next
 * version. Calls the Deployer to do the actual fetches, and to deploy the new version when we have
 * everything ready.
 * <p>
 * We used to support a range of freenet-ext.jar versions. However, supporting ranges creates a lot of
 * complexity, especially with Update Over Mandatory support.
 * <p>
 * File format of dependencies.properties: [module].type=[module type] CLASSPATH means the file must be
 * downloaded, and then added to the classpath in wrapper.conf, before the update can be loaded.
 * <p>
 * OPTIONAL_PRELOAD means we just want to download the file.
 * <p>
 * [module].version=[version number] Can often be parsed from MANIFEST.MF in Jar's, but that is NOT
 * mandatory.
 * <p>
 * [module].filename=[preferred filename] For CLASSPATH, this should be unique, i.e. include the version
 * in the filename, e.g. freenet-ext-29.jar. For OPTIONAL_PRELOAD, we will often overwrite existing
 * files.
 * <p>
 * [module].sha256=[hash in hex] SHA256 hash of the file.
 * <p>
 * [module].filename-regex=[regular expression] Matches filenames for this module. Only required for
 * CLASSPATH. Note that filenames will be toLowerCase()'ed first (but the regex isn't).
 * <p>
 * [module].key=[CHK URI] Where to fetch the file from if we don't have it.
 * <p>
 * [module].size=[decimal size in bytes] Size of the file.
 * <p>
 * Optional:
 * <p>
 * [module].order=[decimal integer order, default is 0] Ordering of CLASSPATH files within the
 * wrapper.conf. E.g. freenet-ext.jar is usually the last element because we want the earlier files to
 * override classes in it.
 * <p>
 * [module].os=[comma delimited list of OS's and pseudo-OS's] OS's: See FileUtil.OperatingSystem: MacOS
 * Linux FreeBSD GenericUnix Windows Pseudo-Os's: ALL_WINDOWS ALL_UNIX ALL_MAC (these correspond to the
 * booleans on FileUtil.OperatingSystem).
 *
 * @author toad
 */
public class MainJarDependenciesChecker {
    ...
    public static boolean validFile(File filename, byte[] expectedHash, long size, boolean executable) {
        ...
        if (filename.length() != size) {
            System.out.println(
                "File exists while updating but length is wrong (" + filename.length() + " should be " +
                size + ") for " + filename);
            return false;
        }
    ...
    public synchronized MainJarDependencies handle(Properties props, int build) {
        try {
            ...
            broken = true;
            Logger.error(this,
                         "MainJarDependencies parsing update dependencies.properties file broke: " + e,
                         e);
            throw e;
        } catch (Error e) {
            broken = true;
    ...
    public boolean cleanup(Properties props, final Deployer deployer, int build) {
        ...
        File[] listMain = new File(".").listFiles(new FileFilter() {
            ...
            public boolean accept(File arg0) {
                ...
                return !name.equals("cryptad.jar") && !name.equals("cryptad.jar.new") &&
        ...
        for (String propName : props.stringPropertyNames()) {
                ...
                if (s.startsWith("OPTIONAL_")) {
                    if (logMINOR) {
                        Logger.minor(MainJarDependencies.class,
                                     "Ignoring non-essential dependency type \"" + s + "\" for \"" +
                                     baseName + "\"");
                    }
                    continue;
                }
            ...
            if (s != null) {
                if (!matchesCurrentArch(s)) {
                    Logger.normal(this,
                                  "Ignoring " + baseName + " as not relevant to this architecture");
                    continue;
                }
            }
            ...
            if (mustBeOnPathNotAScript != null && !isOnPathNotAScript(mustBeOnPathNotAScript)) {
                Logger.normal(this,
                              "Ignoring " + baseName + " because needs \"" + mustBeOnPathNotAScript +
            ...
            if (version == null) {
                Logger.error(MainJarDependencies.class,
                             "dependencies.properties broken? missing version");
                return false;
            }
            File filename = null;
            ...
            if (filename == null) {
                Logger.error(MainJarDependencies.class,
                             "dependencies.properties broken? missing filename");
                return false;
            }
            ...
            try {
                ...
                Logger.error(MainJarDependencies.class,
                             "Unable to parse CHK for " + baseName + ": \"" + s + "\": " + e, e);
                return false;
            }
            ...
            if (size < 0) {
                System.err.println("Unable to update to build " + build +
                                   ": dependencies.properties broken: Broken length for " + baseName +
                                   " : \"" + s + "\"");
                return false;
            }
            ...
            if (s != null) {
                try {
                    ...
                    System.err.println("Unable to update to build " + build +
                                       ": dependencies.properties broken: Broken order for " + baseName +
                                       " : \"" + s + "\"");
                    continue;
                }
            }
            ...
            if (currentFile != null && currentFile.exists() &&
                ...
                // Will be dealt with during update. For now ignore it. Not safe to preload it, since
                // it's on the classpath, whether it exists or not.
                System.out.println("Component " + baseName +
                                   " is using a non-standard file, we cannot serve the file " +
                                   filename +
                                   " via UOM to other nodes. Hence they may not be able to download " +
                                   "the update from us.");
            } else {
                ...
                try {
                    ...
                    deployer.fetch(key, filename, size, expectedHash, new JarFetcherCallback() {
                        ...
                        public void onFailure(FetchException e) {
                            Logger.error(this, "Failed to preload " + file + " from " + key + " : " + e,
                                         e);
                        }
                    ...
                    Logger.error(MainJarDependencies.class,
                                 "Failed to preload " + file + " from " + key + " : " + e, e);
                }
            }
            ...
            if (currentFileVersion == null) {
                continue; // If no version in the current version, no version in any other either,
            }
            // can't reliably detect outdated jars. E.g. freenet-ext.jar up to v29!
        ...
        for (String propName : props.stringPropertyNames()) {
            ...
            if (s == null) {
                ...
                try {
                    ...
                    Logger.error(this, "Unable to parse CHK for " + baseName + ": \"" + s + "\": " + e,
                                 e);
                    maxCHK = null;
                }
            }
            // FIXME where to get the proper folder from? That seems to be an issue in
            //  UpdateDeployContext as well...
            ...
            if (type == DEPENDENCY_TYPE.CLASSPATH) {
                ...
                if (regex == null && type == DEPENDENCY_TYPE.CLASSPATH) {
                    // Not a critical error. Just means we can't clean it up, and can't identify
                    // whether we already have a compatible jar.
                    Logger.error(this, "No " + baseName +
                                       ".filename-regex in dependencies.properties - we will not be " +
                                       "able to clean up old versions of files, and may have to " +
                                       "download the latest version unnecessarily");
            ...
            if (type == DEPENDENCY_TYPE.CLASSPATH) {
                if (p == null) {
                    ...
                    if (maxCHK != null) {
                        try {
                            fetchDependency(maxCHK, new Dependency(currentFile, filename, p, order),
                                            expectedHash, size, true, executable);
                ...
                for (File f : list) {
                    ...
                    if (validFile(f, expectedHash, size, executable)) {
                        // Use it.
                        System.out.println("Found " + name + " - meets requirement for " + baseName +
                                           " for next update.");
            ...
            try {
                fetchDependency(maxCHK, new Dependency(currentFile, filename, p, order), expectedHash,
                                size, type != DEPENDENCY_TYPE.OPTIONAL_PRELOAD, executable);
            } catch (FetchException e) {
                broken = true;
                Logger.error(this, "Failed to start fetch: " + e, e);
                System.err.println(
                    "Failed to start fetch of essential component for next release: " + e);
            }
        }
        if (ready()) {
        ...
        for (String s : split) {
            ...
            if (f.exists() && f.isDirectory()) {
                ...
                if (f.exists() && f.canExecute()) {
                    if (!f.canRead()) {
                        Logger.error(this,
                                     "On path and can execute but not read, so can't check whether it " +
                                     "is a script?!: " + f);
                        return false;
                    }
                    if (f.length() < SCRIPT_HEAD.length) {
                        Logger.error(this, "Found " + toFind + " on path but less than " + SCRIPT_HEAD +
                                           " bytes long, so can't check whether it is a script - will " +
                                           "the shell try the next match? We can't tell whether it is " +
                                           "a script or not ...");
                        return false; // Weird!
                    }
                    try {
    ...
    private boolean parseAtomicMultiFilesWithRestart(Properties props, String name) {
        ...
        for (String propName : props.stringPropertyNames()) {
            ...
            if (s == null) {
                Logger.error(MainJarDependencies.class,
                             "dependencies.properties broken? missing " + fileBase +
            ...
            try {
                ...
                Logger.error(MainJarDependencies.class,
                             "Unable to parse CHK for multi-files replace for " + fileBase + ": \"" + s +
                             "\": " + e, e);
                atomicDeployer.cleanup();
                return false;
            }
            ...
            if (s != null) {
                try {
                    ...
                    Logger.error(MainJarDependencies.class,
                                 "Unable to parse size for multi-files replace for " + fileBase +
                                 ": \"" + s + "\": " + e, e);
                    atomicDeployer.cleanup();
                    return false;
                }
            ...
            if (s == null) {
                ...
                try {
                    ...
                    Logger.error(MainJarDependencies.class,
                                 "Unable to past mustExist \"" + s + "\" for " + fileBase);
                    atomicDeployer.cleanup();
                    return false;
                }
            }
            ...
            if (!filename.exists()) {
                if (mustExist != MUST_EXIST.FALSE) {
                    System.out.println(
                        "Not running multi-file replace " + name + " : File does not exist: " +
                        filename);
                    atomicDeployer.cleanup();
                    return false;
                }
                ...
                if (mustExist == MUST_EXIST.EXACT) {
                    System.out.println(
                        "Not running multi-file replace: Not compatible with old version of " +
            ...
            if (mustBeOnClassPath) {
                ...
                if (f == null) {
                    System.err.println(
                        "Not running multi-file replace: File must be on classpath: " + filename +
                        " for " + name);
                    atomicDeployer.cleanup();
                    return false;
                }
    ...
    private AtomicDeployer createRestartingAtomicDeployer(String name) {
        if (FileUtil.detectedOS.isUnix || FileUtil.detectedOS.isMac) {
            ...
            System.out.println(
                "Multi-file update for " + name + " not supported on unknown non-unix non-windows OS " +
                FileUtil.detectedOS);
            return null;
        }
    }
     ...
     * actually exist, and the caller should check! However, even a non-existent filename may be useful
     * when updating wrapper.conf.
     */
    ...
    private static byte[] parseExpectedHash(String sha256, String baseName) {
        if (sha256 == null) {
            Logger.error(MainJarDependencies.class,
                         "No SHA256 for " + baseName + " in dependencies.properties");
            return null;
        }
        try {
    ...
    private synchronized void fetchDependency(
        FreenetURI chk, Dependency dep, byte[] expectedHash, long expectedSize, boolean essential,
    ...
    interface Deployer {
        ...
        JarFetcher fetch(
            FreenetURI uri, File downloadTo, long expectedLength, byte[] expectedHash,
            JarFetcherCallback cb, int build, boolean essential, boolean executable)
    ...
    class MainJarDependencies {
        MainJarDependencies(TreeSet<Dependency> dependencies, int build) {
            ...
            for (Dependency d : dependencies) {
                ...
                if (File.pathSeparatorChar == ':' && d.oldFilename != null &&
                    d.oldFilename.getName().equalsIgnoreCase("freenet-ext.jar.new")) {
                    // If wrapper.conf currently contains freenet-ext.jar.new, we need to update
                    // wrapper.conf even
                    // on unix. Reason: freenet-ext.jar.new won't be read if it's not the first item
                    // on the classpath,
        ...
        /**
         * True if we must rewrite wrapper.conf, i.e. if any new jars have been added, or new versions of
         * existing jars. Won't be reliably true in case of jars being removed at present. FIXME see
    ...
    class AtomicDeployer {
        ...
        public void onFailure(AtomicDependency dep, FetchException e) {
            ...
            System.err.println(
                "Unable to deploy multi-file update " + name + " because fetch failed for " +
                dep.filename);
            cleanup();
        }
        ...
        boolean innerDeployMultiFileUpdate() {
            synchronized (this) {
                if (failed || !started) {
                    Logger.error(this, "Not deploying: failed=" + failed + " started=" + started,
                                 new Exception("error"));
                    return false;
                }
            }
            ...
            for (AtomicDependency dep : deps) {
                if (!dep.deploy()) {
                    failedDeploy = true;
                    System.err.println(
                        "Unable to update file " + dep.filename + " from " + dep.tempFilename +
                        " - aborting multi-file update deployment " + name);
                    break;
                }
            }
    ...
    private class Downloader implements JarFetcherCallback {
         ...
         */
        Downloader(
            Dependency dep, FreenetURI uri, byte[] expectedHash, long expectedSize, boolean essential,
            boolean executable, int forBuild) throws FetchException {
            fetcher =
                deployer.fetch(uri, dep.newFilename, expectedSize, expectedHash, this, build, essential,
                               executable);
            this.dep = dep;
    ...
    private class AtomicDependency implements JarFetcherCallback {
        ...
        public boolean start(AtomicDeployer myDeployer) {
            ...
            try {
                JarFetcher fetcher =
                    deployer.fetch(key, tempFilename, size, expectedHash, this, build, false,
                                   executable /* we use rename, so ideally we'd like the temp file to
                                   be executable if the target will be */);
                synchronized (this) {
                ...
                Logger.error(this,
                             "Unable to start fetch for " + filename + " from " + key + " size " + size +
            ...
            if (innerDeployMultiFileUpdate()) {
                try { // FIXME use nodeDir
                    if (Runtime.getRuntime()
                               .exec(new String[]{shell.toString(), restartScript.toString()}) == null) {
                        System.err.println(
                            "Unable to start restarter script " + restartScript + " with shell " +
                            shell + " -> cannot deploy multi-file update for " + name);
                        return false;
                    }
                } catch (IOException e) {
                    System.err.println(
                        "Unable to start restarter script " + restartScript + " with shell " + shell +
                        " -> cannot deploy multi-file update for " + name + " : " + e);
                    Logger.error(this,
                                 "Unable to start restarter script " + restartScript + " with shell " +
                                 shell + " -> cannot deploy multi-file update for " + name + " : " + e,
                                 e);
                    return false;
                }
                System.out.println(
    ...
    enum DEPENDENCY_TYPE {
        ...
        CLASSPATH,
        /**
         * A jar we want to put on the classpath but after that we won't update it even if there is a new
         * version. Used for wrapper.jar since we will update it via a separate mechanism, because we
         * have to update other files too. No regex support - must match the exact filename. We do
    ...
    /**
     * Set if the update can't be deployed because the dependencies file is broken. We should wait for an
     * update with a valid file.
     */
