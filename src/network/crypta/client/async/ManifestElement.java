...
public class ManifestElement implements Serializable {
    ...
    public network.crypta.support.api.ManifestElement migrate(BucketFactory bf, ClientContext context) throws ResumeFailedException, IOException {
        if(data == null) {
            if(targetURI == null) throw new ResumeFailedException("Must have either a URI or a redirect");
            return new network.crypta.support.api.ManifestElement(name, fullName, mimeOverride, targetURI);
        } else {
            ...
            return new network.crypta.support.api.ManifestElement(name, fullName, convertedData, mimeOverride, dataSize);
        }
	}
