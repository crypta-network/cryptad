...
public class GenericReadFilterCallback implements FilterCallback, URIProcessor {
    ...
    public GenericReadFilterCallback(
            URI uri, FoundURICallback cb, TagReplacerCallback trc,
    ...
    public GenericReadFilterCallback(
            FreenetURI uri, FoundURICallback cb, TagReplacerCallback trc,
            LinkFilterExceptionProvider linkFilterExceptionProvider) {
        try {
    ...
    /**
     * Sets the {@link BaseL10n l10n provider} to use for translating some error messages. If this method
     * is not called, {@link NodeL10n}’s {@link NodeL10n#getBase() l10n provider} is used.
     * <p>
    ...
    public String processURI(
            String u, String overrideType, String forceSchemeHostAndPort,
    ...
    private String finishProcess(
        ...
        try {
            ...
            if (logMINOR) {
                Logger.minor(this,
                        "Returning " + uri.toASCIIString() + " from " + path + " from baseURI=" +
                                baseURI + " stripped base uri=" + strippedBaseURI.toString());
            }
            return uri.toASCIIString();
        } catch (URISyntaxException e) {
            Logger.error(this,
                    "Could not parse own URI: path=" + path + ", typeOverride=" + typeOverride +
                            ", frag=" + u.getFragment() + " : " + e, e);
            String p = path;
            if (typeOverride != null) {
                p += "?type=" + typeOverride;
            }
