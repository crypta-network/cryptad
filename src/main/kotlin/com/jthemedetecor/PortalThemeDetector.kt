...
class PortalThemeDetector : OsThemeDetector(), Closeable {
  ...
  init {
    ...
    try {
      try { settings.ReadOne(APPEARANCE, COLOR_SCHEME) } catch (_: DBusExecutionException) { settings.Read(APPEARANCE, COLOR_SCHEME) }
    } catch (t: Throwable) {
  ...
  override fun isDark(): Boolean {
    return try {
      val v = try { settings.ReadOne(APPEARANCE, COLOR_SCHEME) } catch (_: DBusExecutionException) { settings.Read(APPEARANCE, COLOR_SCHEME) }
      mapVariantToDark(v)
      ...
      try {
        conn.addSigHandler(PortalSettings.SettingChanged::class.java) { sig ->
          if (sig.namespace == APPEARANCE && sig.key == COLOR_SCHEME) {
            ...
            listeners.forEach { l ->
              try { l.accept(isDark) } catch (_: RuntimeException) {}
            }
          }
        }
      } catch (_: DBusConnectionException) { /* ignore, no live updates */ }
    }
  }
