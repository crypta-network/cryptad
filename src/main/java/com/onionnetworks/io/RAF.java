...
public class RAF {
  ...
  /**
   * This version of renameTo() attempts to mimic the behavior of the unix 'mv' command rather than
   * File.renameTo. This means that the destFile will automatically be deleted if it exists and if
   * we are unable to mv the file directly because they are on different file systems then we will
   * manually copy and delete the original.
   *
   * <p>If there is an exception during the copy then we will attempt to delete the new copy and
   * revert back to the old copy. There is a very slight chance that after reverting, the original
   * copy may be unusable. This would probably only happen with a file system that is experiencing
   * IO problems, in which case you are going to have problems anyway.
   *
   * <p>There is also a very rare chance that during a failed copy, we will be unable to delete a
   * partially written destination file and it will remain on disk. This could happen on a directory
   * with "drop box" semantics where you can only create and write, and not delete.
   */
    ...
    try {
      ...
      if (!f.renameTo(destFile)) {
        ...
        try {
          ...
          throw new IOException("Unable to move " + f + " to " + destFile + " : " + e.getMessage());
        }
