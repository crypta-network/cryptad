...
public class Buffer {
  ...
  /**
   * Calling this defeats the purpose of reusing byte arrays, so only call this when you have to get
   * a byte[].
   */
  ...
  /**
   * If two buffers are the same length and contain the same data, then this method will return
  ...
  /**
   * Uncomment this if you're not a coward. / public int hashCode() { int retval = 0; for (int
   * i=0;i<len;i++) { retval |= ((int)(b[off+i])) << (8 * (i & 4)); } return retval; }
   */
