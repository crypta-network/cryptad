...
public class Util {
   ...
   *
   * <p>This method is meant to be a clone of the functionality of the C bzero function.
   *
  ...
  public static final void arraycopy(
      char[] chars, int charOff, byte[] bytes, int byteOff, int numBytes) {
  ...
  /**
   * Dumps a byte array to an UNIX style hex dump. This isn't terribly efficient so you should
  ...
  public static final void arraycopy(
      byte[] bytes, int byteOff, char[] chars, int charOff, int numBytes) {
    int indexCounter = byteOff;
    int loopMax = numBytes / 2 + charOff;
    for (int i = charOff; i < loopMax; i++) {
   ...
   */
  public static final Method getPublicMethod(Class clazz, String name, Class[] types)
    ...
    for (int i = 0; i < methods.length; i++) {
      if (Modifier.isPublic(methods[i].getModifiers())
        ...
        for (int j = 0; j < types.length; j++) {
          if (!methods[i].getParameterTypes()[j].isAssignableFrom(types[j])) {
            break;
