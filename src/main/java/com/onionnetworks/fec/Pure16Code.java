 ...
 * This is heavily dervied from Luigi Rizzos original C implementation. See the file "LICENSE" along
 * with this distribution for additional copyright information.
 *
...
public class Pure16Code extends PureCode {
  ...
  /**
   * Notes about large N support: you can just generate the top k*k vandermonde matrix, call it V,
   * then invert it, then when you have k blocks generate a matrix M with the k rows you need <r_i>
   * and E = M* V^{-1} is the encoding matrix for the systematic code which you then need to invert
   * to perform the decoding. Probably there is a fast way to invert E given that M is also a
   * vandermonde matrix so it is "easy" to compute M^{-1}
   */
  ...
  /**
   * encode accepts as input pointers to n data packets of size sz, and produces as output a packet
   * pointed to by fec, computed with index "index".
   */
