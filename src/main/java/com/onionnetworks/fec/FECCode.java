...
/**
 * This class provides the main API/SPI for the FEC library. You cannot construct an FECCode
 * directly, rather you must use an FECCodeFactory.
 *
 * <p>For example: <code>
 *   int k = 32;
 *   int n = 256;
 *   FECCode code = FECCodeFactory.getDefault().createFECCode(k,n);
 * </code> All FEC implementations will sublcass FECCode and implement the encode/decode methods.
 * All codes implemented by this interface are assumed to be systematic codes which means that the
 * first k repair packets will be the same as the original source packets.
 *
...
public abstract class FECCode {
   ...
   * @param n The number of packets that the source packets will be encoded to.
   */
   ...
   *
   * @param src An array of <code>k</code> byte[]'s that contain the source packets to be encoded.
   *     Often these byte[]'s are actually references to a single byte[] that contains the entire
   *     source block, then you must simply vary the srcOff's to pass it in in this fashion. src[0]
   *     will point to the 1st packet, src[1] to the second, etc.
   * @param srcOff An array of integers which specifies the offset of each each packet within its
   *     associated byte[].
   * @param repair Much like src, variable points to a number of buffers to which the encoded repair
   *     packets will be written. This array should be the same length as repairOff and index.
   * @param repairOff This is the repair analog to srcOff.
   * @param index This int[] specifies the indexes of the packets to be encoded and written to
   *     <code>repair</code>. These indexes must be between 0..n (should probably be k..n, because
   *     encoding < k is a NOP)
   * @param packetLength the packetLength in bytes. All of the buffers in src and repair are assumed
   *     to be this long.
   */
   ...
   * they are shuffled so that packets that are original source packets (index < k) are positioned
   * so that their index within the byte[][] is the same as their packet index. If the <code>
   ...
   * wrapping an underlying byte[] then the shuffling proceedure may bring the byte[][] out of sync
   ...
   *
   * @param pkts An array of <code>k</code> byte[]'s that contain the repair packets to be decoded.
   *     The decoding proceedure will copy the decoded data into the byte[]'s that are provided and
   *     will place them in order within the byte[][]. If the byte[][] is already properly shuffled
   *     then the byte[]'s will not be moved around in the byte[][].
   * @param pktsOff An array of integers which specifies the offset of each each packet within its
   *     associated byte[].
   * @param index This int[] specifies the indexes of the packets to be decoded. These indexes must
   *     be between 0..n
   * @param packetLength the packetLength in bytes. All of the buffers in pkts are assumed to be
   *     this long.
   */
   ...
   * transition is expensive.
   *
   * @param src An array of <code>k</code> Buffers that contain the source packets to be encoded.
   *     Often these Buffers are actually references to a single byte[] that contains the entire
   *     source block.
   * @param repair Much like src, variable points to a number of Buffers to which the encoded repair
   *     packets will be written. This array should be the same length as index.
   * @param index This int[] specifies the indexes of the packets to be encoded and written to
   *     <code>repair</code>. These indexes must be between 0..n (should probably be k..n, because
   *     encoding < k is a NOP)
   */
   ...
   * This method takes an array of encoded packets and decodes them. Before the packets are decoded,
   * they are shuffled so that packets that are original source packets (index < k) are positioned
   * so that their index within the byte[][] is the same as their packet index.
   *
   * <p>We shuffle the packets using the copy mechanism to allow API users to be guarenteed that the
   ...
   * place.
   *
   * @param pkts An array of <code>k</code> Buffers that contain the repair packets to be decoded.
   *     The decoding proceedure will copy the decoded data into the Buffers that are provided and
   *     will place them in order within the Buffer[]. If the Buffers are already properly shuffled
  ...
  /**
   * Move packets with index < k into their position. This method copies the data using
   * System.arraycopy rather than modifying the Buffer[].
   */
