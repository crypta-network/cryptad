...
/**
 * This class provides the majority of the logic for the pure Java implementation of the vandermonde
 * FEC codes. This code is heavily derived from Luigi Rizzo's original C implementation. Copyright
 * information can be found in the 'LICENSE' file that comes with this distribution.
 *
...
public class FECMath {
   ...
   * PDA, especially because it can be pre-initialized an put into a ROM!), otherwhise we use a
   * table of logarithms.
   */
  ...
  /**
   * Generate GF(2**m) from the irreducible polynomial p(X) in p[0]..p[m] Lookup tables:
   * index->polynomial form gf_exp[] contains j= \alpha^i; polynomial form -> index form gf_log[ j =
   * \alpha^i ] = i \alpha=x is the primitive element of GF(2^m)
   *
   * <p>For efficiency, gf_exp[] has size 2*gfSize, so that a simple multiplication of two numbers
   * can be resolved without calling modnn
   */
