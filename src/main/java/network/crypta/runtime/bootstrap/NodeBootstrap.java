 ...
 * #setupRandomSources(RandomSource, RandomSource, HttpShellContainer, NativeThread,
 * ProgramDirectory)} once the HTTP shell container and entropy thread are available. The class also
...
public final class NodeBootstrap {
   ...
   * and notifies the HTTP shell container so the UI can drop the entropy warning. For the fast weak
   ...
   * @param toadlets HTTP shell container providing startup readiness notification.
   ...
   * HttpShellContainer, NativeThread, ProgramDirectory)} and is expected to remain stable for the
   ...
   * HttpShellContainer, NativeThread, ProgramDirectory)} completes or when {@link #markPrngReady()}
   * is called explicitly. The value is volatile and intended for low-cost readiness checks from
   * other threads, such as the startup UI.
   *
   ...
   * alternate means or after out-of-band checks. It does not notify the HTTP shell container or
   * perform any additional initialization; it only updates the volatile flag.
   */
