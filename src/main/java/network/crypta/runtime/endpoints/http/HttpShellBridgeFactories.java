 ...
 *
 * <p>This helper keeps knowledge of the current bridge implementations inside the endpoint-owned
 * package while letting higher-level runtime/bootstrap code depend only on the neutral seam types
 * from {@code network.crypta.runtime.http}. Bootstrap code calls these accessors when it wants the
 * historical daemon wiring, but the neutral seam package stays free of references back to
 * endpoint-owned bridge classes. That separation keeps ownership boundaries clear without changing
...
public final class HttpShellBridgeFactories {
   ...
   *
   * @return factory that creates the current endpoint-backed HTTP shell container bridge without
