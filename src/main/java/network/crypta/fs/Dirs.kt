...
abstract class BaseDirs(
    ...
    private fun cliOverrides(): Overrides = Overrides(
...
class AppDirs(
    ...
    constructor(cliOverrides: Map<String, String>) : this(
        System.getenv(),
        System.getProperties().entries.associate { (k, v) -> k.toString() to v.toString() },
        cliOverrides,
        AppEnv(System.getenv())
            ...
            runtimeBase = when {
                appEnv.isFlatpak() -> {
                    ...
                    (xdgRuntime ?: Paths.get("/run/user", (systemProperties["user.name"] ?: "0")))
                        .resolve("app").resolve(appId).resolve(APP_RUNTIME_SUBPATH)
                }
                appEnv.isSnap() -> {
                ...
                xdgRuntime != null -> xdgRuntime.resolve(APP_RUNTIME_SUBPATH)
                    ...
                    val uidBased = Paths.get("/run/user").resolve(System.getProperty("user.name") ?: "0").resolve(APP_RUNTIME_SUBPATH)
...
class ServiceDirs(
    ...
    constructor(env: Map<String, String>, appEnv: AppEnv) : this(
        env,
        System.getProperties().entries.associate { (k, v) -> k.toString() to v.toString() },
        emptyMap(),
        appEnv
    ...
    override fun computeBase(): Resolved {
        return when {
            appEnv.isWindows() -> {
                val programData = env["PROGRAMDATA"]
                    ?: Paths.get(systemProperties[USER_HOME] ?: System.getProperty(USER_HOME), "AppData", "Local").toString()
    ...
    override fun envOverrides(): Overrides {
        ...
        return if (!appEnv.isLinux()) Overrides() else Overrides(
