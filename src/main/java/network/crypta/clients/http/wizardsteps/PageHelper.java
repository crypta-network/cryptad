...
/**
 * Provides a page content node, forms, and InfoBoxes. Used to wrap ToadletContext access away from
 * Wizard Steps. A new one should be constructed each time a step is run.
 */
public class PageHelper {
  ...
  /**
   * Generates a PageMaker with the appropriate arguments for the wizard. (Ex hiding status and nav
   * bars) This is so that steps can determine their page title at runtime instead of being limited
   * to one. This is needed for the physical security page.
   *
   ...
   *
   * <p>If you are getting the outer node with the sole purpose of generating its HTML, use {@link
   * #generate()} instead.
   *
   ...
   *
   * <p>{@link #getPageContent(String)} must be called before this method.
   *
  ...
  /**
   * Generates a form that includes persistence for inter-step fields. This is currently opennet,
   * preset, and step. Opennet is whether the user enabled opennet, preset is what preset they're
   * using, and step is what POST step will be used to process the form.
   *
