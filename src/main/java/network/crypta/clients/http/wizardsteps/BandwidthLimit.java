 ...
 *
 * @param downBytes Download limit in bytes.
 * @param upBytes   Upload limit in bytes.
 */
public record BandwidthLimit(long downBytes, long upBytes, String descriptionKey, boolean maybeDefault) {
    ...
    public BandwidthLimit(long bytesPerMonth) {
        ...
        this((long) Math.ceil(dllimit), (long) Math.ceil(ullimit), "Monthly bandwidth limit", false);
    }
}
