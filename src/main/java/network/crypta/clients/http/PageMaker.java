...
public final class PageMaker {
    ...
    public enum THEME {
        ...
        CLEAN_CLASSIC("clean-classic", "Clean (Classic menu)", "Clean theme with a classic menu.", false,
                      false),
        CLEAN_DROPDOWN("clean-dropdown", "Clean (Dropdown menu)", "Clean theme with a dropdown menu.",
                       false, false),
        CLEAN_STATIC("clean-static", "Clean (Static menu)", "Clean theme with a static menu.", false,
                     false),
        ...
        WINTERFACEY("winterfacey", "Winterfacey", "2016th-theme, based on Winterface (Bootstrap)", true,
                    false);


        public static final String[] possibleValues =
            {BOXED.code, BOXED_CLASSIC.code, BOXED_DROPDOWN.code, BOXED_DYNAMIC.code, BOXED_STATIC.code,
                CLEAN.code, CLEAN_CLASSIC.code, CLEAN_DROPDOWN.code, CLEAN_STATIC.code, CLEAN_TOP.code,
                GRAYANDBLUE.code, GRAYANDBLUE_DYNAMIC.code, GRAYANDBLUE_DROPDOWN.code,
                GRAYANDBLUE_STATIC.code, GRAYANDBLUE_TOP.code, SKY.code, SKY_CLASSIC.code,
                SKY_DROPDOWN.code, SKY_DYNAMIC.code, SKY_STATIC.code, SKY_DARK_STATIC.code,
                MINIMALBLUE.code, MINIMALISTIC.code, RABBIT_HOLE.code, WINTERFACEY.code};

        public final String code;  // the internal name
        public final String name;  // the name in "human form"
        public final String description; // description
        /**
         * If true, the activelinks will appear on the welcome page, whether the user has enabled them or
         * not.
         */
        ...
        THEME(
            String code, String name, String description, boolean forceActivelinks,
    ...
    /**
     * Filters a given string so that it will be a valid CSS identifier. It replaces all characters that
     * are not a dash, underscore, or alphanumeric with an underscore. If the first character is a dash
     * and the second character is not a letter or underscore, replaces the second character with an
     * underscore. This filter is overly strict as it does not allow non-ASCII characters or escapes. If
     * the given string is below two characters in length, it appends underscores until it is not.
     *
    ...
    public synchronized void addNavigationLink(
        String menutext, String path, String name, String title, boolean fullOnly,
    ...
    /**
     * Generates an FProxy template page with optional navigation bar and status information suitable for
     * adding content to.
     *
     * @param title                 Title of the page.
     * @param renderNavigationLinks Whether to render navigation links.
    ...
    public PageNode getPageNode(
        ...
        return getPageNode(title, ctx,
                           new RenderParameters().renderNavigationLinks(renderNavigationLinks)
                                                 .renderStatus(renderStatus).renderModeSwitch(true));
    }
     ...
     *
     * @param title            Title of the page.
     * @param ctx              ToadletContext to use to render the page. Can be null, e.g. if the HTML is
     *                         not being generated as part of a toadlet request, for example if it's
     *                         using the old FredPluginHTTP interface.
    ...
    public PageNode getPageNode(String title, ToadletContext ctx, RenderParameters renderParameters) {
		...
		if (override != null) {
			...
			headNode.addChild("link", new String[]{"rel", "href", "type", "title"},
							  new String[]{"stylesheet", "/static/themes/" + theme.code + "/theme.css",
								  "text/css", theme.code});
		}
        ...
        if (sendAllThemes) {
            for (THEME t : THEME.values()) {
                ...
                headNode.addChild("link", new String[]{"rel", "href", "type", "media", "title"},
        ...
        if (ctx != null && ctx.getContainer().isFProxyJavascriptEnabled()) {
            ...
            if (themeJsUrl != null) {
                headNode.addChild("script", new String[]{"type", "language", "src"},
                                  new String[]{"text/javascript", "javascript",
                                      "/static/themes/" + theme.code + "/script.js"});
            }
        }

        boolean webPushingEnabled = ctx != null && ctx.getContainer().isFProxyJavascriptEnabled() &&
		...
		if (webPushingEnabled) {
			headNode.addChild("script", new String[]{"type", "language", "src"},
							  new String[]{"text/javascript", "javascript",
								  "/static/freenetjs/freenetjs.nocache.js"});
		}
        ...
        HTMLNode bodyNode = htmlNode.addChild("body", new String[]{"class", "id"},
                                              new String[]{"fproxy-page",
                                                  filterCSSIdentifier("page-" + activePath)});
        //Add a hidden input that has the request's id
		if (webPushingEnabled) {
			bodyNode.addChild("input", new String[]{"type", "name", "value", "id"},
							  new String[]{"hidden", "requestId", ctx.getUniqueId(), "requestId"});
		}

        // Add the client-side localization only when pushing is enabled
        if (webPushingEnabled) {
            bodyNode.addChild("script", new String[]{"type", "language"},
                              new String[]{"text/javascript", "javascript"})
        ...
        if (renderParameters.isRenderStatus() && fullAccess) {
            ...
            statusBarDiv.addChild("div", "id", "statusbar-language")
                        .addChild("a", "href", "/config/node#l10n",
            ...
            if (node != null && node.getClientCore() != null) {
                ...
                final HTMLNode secLevels = statusBarDiv.addChild("div", "id", "statusbar-seclevels",
                                                                 NodeL10n.getBase().getString(
                                                                     "SecurityLevels.statusBarPrefix"));
                ...
                final HTMLNode network = secLevels.addChild("a", "href", "/seclevels/",
                                                            ...
                                                            "\u00a0");
                network.addAttribute("title", NodeL10n.getBase().getString(
                    "SecurityLevels.networkThreatLevelShort"));
                network.addAttribute("class", node.getSecurityLevels().getNetworkThreatLevel().toString()
                                                  .toLowerCase());
                ...
                physical.addAttribute("title", NodeL10n.getBase().getString(
                    "SecurityLevels.physicalThreatLevelShort"));
                physical.addAttribute("class",
                                      node.getSecurityLevels().getPhysicalThreatLevel().toString()
                                          .toLowerCase());
                ...
                final int totalPeers =
                    (node.getOpennet() == null) ? (darknetTotal > 0 ? darknetTotal : Integer.MAX_VALUE) :
                ...
                progressBar.addChild("div", new String[]{"class", "style"}, new String[]{
                    "progressbar-done progressbar-peers " + additionalClass,
                    "width: " + Math.min(100, Math.floor(100 * connectedRatio)) + "%;"});

                progressBar.addChild("div", new String[]{"class", "title"},
                                     ...
                                     connectedPeers +
                                     ((totalPeers != Integer.MAX_VALUE) ? " / " + totalPeers : ""));
            }
        }
                ...
                for (SubMenu menu : menuList) {
                    ...
                    for (String navigationLink : fullAccess ? menu.navigationLinkTexts :
                        ...
                        if (l10n != null) {
                            ...
                            sublistItem.addAttribute("id",
                                                     getPluginL10nCSSIdentifier(l10n, navigationTitle));
						...
						if (navigationTitle != null) {
							sublistItem.addChild("a", new String[]{"href", "title"},
                    ...
                    if (nonEmpty) {
                        ...
                        listItem.addChild("a", new String[]{"href", "title"},
    ...
    private HTMLNode getOverrideContent() {
        return new HTMLNode("link", new String[]{"rel", "href", "type", "media", "title"},
                            new String[]{"stylesheet", override, "text/css", "screen", "custom"});
    }

    /**
     * Bundles parameters that are used to create the page node. The default for the render parameters is
     * to include all optional render tasks. Individual tasks may be enabled or disabled by calling the
     * appropriate methods which returns a new {@link RenderParameters} object as
     * {@link RenderParameters} are immutable.
     *
    ...
    public static class RenderParameters {
         ...
         *
         * @return {@code true} if the status bar should be included in the page, {@code false} otherwise
         */
