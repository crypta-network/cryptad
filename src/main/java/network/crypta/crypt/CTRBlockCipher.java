 ...
 * sorting out the policy files issues. Bouncycastle is MIT X licensed i.e. GPL compatible.
 */
...
public class CTRBlockCipher {
  ...
  /** Block size in bytes. Equal to IV.length = counter.length = counterOut.length. */
  private final int blockSize;

  /** Initialization vector, equal to the initial value of the plaintext counter. */
  private final byte[] IV;

  /**
   * The plaintext block counter. This is incremented (from [31] backwards) after each block
   * encryption.
   */
  private final byte[] counter;

  /**
   * The ciphertext block counter. This is the result of encrypting the plaintext block counter. It
   * is XOR'ed with the plaintext to get the ciphertext.
   */
  ...
  /**
   * Initialize the cipher with an IV. Must only be called once for any given IV!
   *
   * @param iv The initialization vector. This is the initial value of the plaintext counter. The
   *     plaintext is XORed with a sequence of bytes consisting of the encryption of successive
  ...
  public void init(byte[] iv, int offset, int length) throws IllegalArgumentException {
    if (length != IV.length) throw new IllegalArgumentException();
