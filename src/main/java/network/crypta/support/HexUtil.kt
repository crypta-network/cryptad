...
/**
 * Provides utility methods for conversions between hexadecimal strings, byte arrays, BitSets, and
 * BigIntegers. Also includes I/O operations for BigIntegers.
 */
...
/**
 * Converts a subsection of a byte array into hexadecimal characters and appends them to a
 * StringBuilder.
 *
 * @param bs The source byte array.
 * @param off The starting offset within the byte array.
...
/**
 * Converts a hexadecimal string into a new byte array with a specified leading offset. The
 * resulting array will be prefixed with `off` zero-bytes.
 *
 * @param s The hexadecimal string to convert.
 * @param off The number of zero-bytes to prefix to the output array.
 * @return A new byte array containing `off` zero-bytes followed by the decoded bytes from the
 *   string.
 */
...
/**
 * Decodes a hexadecimal string into a provided byte array at a given offset. If the string has an
 * odd length, it is implicitly prefixed with a '0'.
 *
 * @param s The hexadecimal string to convert.
 ...
 * @throws IndexOutOfBoundsException if the output buffer `out` is too small to hold the decoded
 *   bytes.
 */
...
/**
 * Writes a non-negative BigInteger to a DataOutputStream. The BigInteger is written as a short
 * representing the length, followed by the byte array of the BigInteger.
 *
 ...
 * @throws IllegalStateException if the BigInteger's byte array is too long (exceeds
 *   Short.MAX_VALUE).
 */
 ...
 * @throws IOException if the length prefix is invalid, an I/O error occurs, or the end of the
...
/**
 * Converts a BigInteger to its hexadecimal string representation. This is a convenience alias for
 * [toHexString].
 *
