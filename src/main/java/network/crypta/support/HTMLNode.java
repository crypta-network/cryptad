...
public class HTMLNode implements XMLCharacterClasses, Cloneable {
  ...
  /**
   * Text to be inserted between tags, or possibly raw HTML. Only non-null if name is "#" (= text)
   * or "%" (= raw HTML). Otherwise the constructor will allocate a separate child node to contain
   * it.
   */
  ...
  /**
   * Tests an HTML element name to determine if it is one of the elements permitted to be empty in
   * the XHTML spec ( http://www.w3.org/TR/xhtml1/ )
   *
  ...
  /**
   * Tests an HTML element to determine if we should add a newline after the opening tag for
   * readability
   *
   * @param name The name of the html element
  ...
  /**
   * Tests an HTML element to determine if we should add a newline after the closing tag for
   * readability. All tags with newlines after the opening tag also get newlines after the closing
   * tag.
   *
   * @param name The name of the html element
   * @return True if we should add a newline after the opening tag
  ...
  /**
   * Returns a properly formatted closing angle bracket to complete an open tag of a named html
   * element
   *
  ...
  protected boolean checkNamePattern(String str) {
    ...
    // Regex-based match. Probably more expensive, and problems (infinite recursion in
    // Pattern$6.isSatisfiedBy) have been seen in practice.
    // Oddly these problems were seen where the answer is almost certainly in the first matcher,
    // because the tag name was "html"...
   ...
   * @return The added node. You can add more tags inside it with addChild(), or add attributes with
   *     addAttribute() etc. If you render the parent tag with generate(), it will include this tag
   *     in its output.
   */
   ...
   *
   * @return The name of the first "real" tag, or <code>null</code> if no "real" tag could be found
   */
  ...
  public StringBuilder generate(StringBuilder tagBuffer, int indentDepth) {
    ...
    // Perhaps this should be something else, but since I don't know if '#' was not just arbitrary
    // chosen, I'll just pick '%'
