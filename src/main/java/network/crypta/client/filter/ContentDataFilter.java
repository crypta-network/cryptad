 ...
 * called a white-list strategy) is strongly preferred over attempting to strip known bad
 * constructs.
 *
 ...
 * input.available() == 0} implies end of stream. They should also avoid unbounded buffering and
 * should propagate I/O failures promptly while keeping partially produced output in a consistent
 * state. Implementations are generally stateless and reusable; if state is kept per invocation,
...
public interface ContentDataFilter {
   ...
   * }</pre>
   *
   * @param input the source of potentially unsafe bytes to validate and sanitize; may be network or
   *     pipe backed and must be read in a streaming manner without relying on {@code available()}.
