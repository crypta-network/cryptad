...
public class ClientRequestScheduler implements RequestScheduler {
  ...
  /**
   * Creates a new scheduler instance for the given mode (inserts vs. GETs), key type, and
   * real-time behavior.
   *
   ...
   * to registering the requests themselves. When {@code noCheckStore} is {@code false}, each
   * getter is first queued for a store-check via the provided {@code blocks}; otherwise, getters
   * are registered immediately if they are not cancelled and not on cooldown.
   *
   ...
   * @param blocks block set context passed to store checks; ignored when {@code getters} is
   *     {@code null} or {@code noCheckStore} is {@code true}
