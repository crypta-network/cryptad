...
import network.crypta.node.updater.MainJarDependenciesChecker.Deployer;
import network.crypta.node.updater.MainJarDependenciesChecker.JarFetcher;
...
public class MainJarUpdater extends NodeUpdater implements Deployer {
    ...
    MainJarUpdater(
        NodeUpdateManager manager, FreenetURI URI, int current, int min, int max,
    ...
    public JarFetcher fetch(
        ...
        DependencyJarFetcher fetcher =
            new DependencyJarFetcher(downloadTo, uri, expectedLength, expectedHash, cb, essential,
                                     executable);
    ...
    private class DependencyJarFetcher
        ...
        DependencyJarFetcher(
            File filename, FreenetURI chk, long expectedLength, byte[] expectedHash,
            ...
            try {
                ...
                throw new FetchException(FetchExceptionMode.BUCKET_ERROR,
                                         "Cannot create temp file for " + filename + " in " + parent +
                                         " - disk full? permissions problem?");
            }
            getter =
                new ClientGetter(this, chk, myCtx, RequestStarter.IMMEDIATE_SPLITFILE_PRIORITY_CLASS,
        ...
        public void onSuccess(FetchResult result, ClientGetter state) {
            ...
            if (!MainJarDependenciesChecker.validFile(tempFile, expectedHash, expectedLength,
                                                      executable)) {
                Logger.error(this, "Unable to download dependency " + filename +
                                   " : not the expected size or hash!");
				...
				if (cb != null) {
					cb.onFailure(new FetchException(FetchExceptionMode.BUCKET_ERROR,
													"Downloaded jar from Freenet but failed " +
													"consistency check: " + tempFile + " length " +
													tempFile.length() + " "));
				}
                tempFile.delete();
                return;
            ...
            if (!FileUtil.moveTo(tempFile, filename)) {
                ...
                System.err.println(
                    "Download of " + filename + " for update failed because cannot rename from " +
                    tempFile);
				if (cb != null) {
					cb.onFailure(new FetchException(FetchExceptionMode.BUCKET_ERROR,
													"Unable to rename temp file " + tempFile + " to " +
													filename));
				}
                tempFile.delete();
                return;
        ...
        public synchronized HTMLNode renderRow() {
			...
			if (uomFetcher != null) {
				...
				row.addChild(
					QueueToadlet.createProgressCell(false, true, COMPRESS_STATE.WORKING, 0, 0, 0, 0, 0,
													false, false));
			} else {
        ...
        public void fetchFromUOM() {
            ...
            UOMDependencyFetcher f = manager.getUpdateOverMandatory()
                                            .fetchDependency(expectedHash, expectedLength, filename,
                                                             executable,
            ...
            synchronized (this) {
                if (uomFetcher != null) {
                    Logger.error(this, "Started UOMFetcher twice for " + filename,
                                 new Exception("error"));
                    return;
                }
                uomFetcher = f;
