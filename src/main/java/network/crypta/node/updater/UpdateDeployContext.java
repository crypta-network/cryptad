...
import network.crypta.node.updater.MainJarDependenciesChecker.Dependency;
...
public class UpdateDeployContext {
    ...
    UpdateDeployContext(MainJarDependencies deps) throws UpdaterParserException {
        ...
        backupMainJar = new File(mainJar.getParent(), "cryptad.jar.bak");
    }
    ...
    public static CHANGED tryIncreaseMemoryLimit(int extraMemoryMB, String markerComment) {
        ...
        try (FileInputStream fis = new FileInputStream(oldConfig);
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (line.startsWith("wrapper.java.maxmemory=")) {
                    try {
                        int memoryLimit =
                            Integer.parseInt(line.substring("wrapper.java.maxmemory=".length()));
                        int newMemoryLimit = memoryLimit + extraMemoryMB;
                        // There have been some cases where really high limits have caused the JVM to
                        // do bad things.
                        if (NodeStarter.isSomething32bits() && newMemoryLimit > 1408) {
                            Logger.error(UpdateDeployContext.class,
                                         "We've detected a 32bit JVM so we're refusing to set " +
                                         "maxmemory to " + newMemoryLimit);
                            newMemoryLimit = 1408;
                        }
        ...
        if (success) {
            if (!newConfig.renameTo(oldConfig)) {
                if (!oldConfig.delete()) {
                    System.err.println(
                        "Unable to move rewritten wrapper.conf with new memory limit " + newConfig +
                        " over old config " + oldConfig + " : unable to delete old config");
                    return CHANGED.FAIL;
                }
                if (!newConfig.renameTo(oldConfig)) {
                    System.err.println(
                        "Old wrapper.conf deleted but new wrapper.conf cannot be renamed!");
                    System.err.println(
    ...
    void rewriteWrapperConf(boolean writtenNewJar)
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (lowcaseLine.startsWith("wrapper.java.classpath.")) {
                ...
                if (idx != -1) {
                    ...
                    dontWrite = true;
                    if (rhs.equals("cryptad.jar") || rhs.equals("cryptad.jar.new") ||
                        rhs.equals("cryptad-stable-latest.jar") ||
                        rhs.equals("cryptad-stable-latest.jar.new") ||
                        rhs.equals("cryptad-testing-latest.jar") ||
                        rhs.equals("cryptad-testing-latest.jar.new")) {
                        if (writtenNewJar) {
                            mainRHS = newMain;
                        } else {
                            mainRHS = rhs;
        ...
        if (mainRHS == null) {
            throw new UpdaterParserException(l10n("updateFailedNonStandardConfig", new String[]{"main"},
        ...
        if (!writtenPrivateModulesOpens && JVMVersion.supportsModules()) {
            ...
            bw.write(
                "wrapper.java.additional." + count + "=--add-opens=java.base/java.lang=ALL-UNNAMED" +
                '\n');
            count++;
            // Unable to make public int java.util.Collections$UnmodifiableCollection.size() accessible
            bw.write(
                "wrapper.java.additional." + count + "=--add-opens=java.base/java.util=ALL-UNNAMED" +
                '\n');
            count++;
        ...
        System.err.println(
            "Rewritten wrapper.conf for build " + deps.build + " and " + deps.dependencies.size() +
            " dependencies.");
    }
