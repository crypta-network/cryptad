...
public class NodeUpdateManager {
  ...
  public void maybeSendUOMAnnounce(PeerNode peer) {
    ...
    try {
      if (Version.isBuildAtLeast(peer.getNodeName(), peer.getBuildNumber(), TRANSITION_VERSION)) {
        if (shouldSend || hasBeenBlown) peer.sendAsync(getNewUOMAnnouncement(size), null, ctr);
      }
  ...
  private void deployUpdate() {
    ...
    try {
      ...
      node.getClientCore()
          .getAlerts()
          .register(new SimpleUserAlert(false, error, error, error, UserAlert.CRITICAL_ERROR));
    } finally {
      if (started && !success) {
        Bucket toFree = null;
        synchronized (this) {
