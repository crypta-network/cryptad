...
/**
 * Runnable thread to retrieve node thread's information and compiling it into an array of NodeThreadInfo
 * objects.
 */
public class DefaultThreadDiagnostics implements Runnable, ThreadDiagnostics {
    ...
    @Override
    public void run() {
        List<NodeThreadInfo> threads = Arrays.stream(nodeStats.getThreads()).filter(Objects::nonNull)
                                             .filter(thread -> thread.getThreadGroup() != null)
                                             .filter(thread -> getJobId(thread) != 0).map(
                thread -> new NodeThreadInfo(thread.getId(), getJobId(thread), getCpuTimeDelta(thread),
                                             getJobName(thread), thread.getPriority(),
                                             thread.getThreadGroup().getName(),
                                             thread.getState().toString())).collect(Collectors.toList());
    ...
    private void purgeInactiveThreads(List<NodeThreadInfo> threads) {
        List<Long> activeThreads =
            threads.stream().map(NodeThreadInfo::getJobId) // job id might be the same as thread id
