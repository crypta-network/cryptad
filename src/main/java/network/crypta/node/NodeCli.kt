 ...
 *
 * Provides a user-friendly CLI with standard help/version flags and
 * clear options for configuring directory overrides and service mode.
 *
 * Usage examples:
...
@Command(
    ...
    header = [
        "Crypta: Distributed, encrypted, censorship-resistant datastore",
        ""
    ],
...
class NodeCli {
    ...
    @Option(names = ["-c", "--config-file"], paramLabel = "FILE", description = [
        "Path to configuration file (cryptad.ini). Overrides --config-dir."])
    var configFileOpt: java.io.File? = null

    /** Optional explicit config file path (positional). */
    @Parameters(arity = "0..1", paramLabel = "FILE", description = [
    ...
    @Option(names = ["-C", "--config-dir"], paramLabel = "PATH", description = [
        "Override configuration directory where cryptad.ini is located."])
    var configDir: String? = null

    /** Override the data directory. */
    @Option(names = ["-d", "--data-dir"], paramLabel = "PATH", description = [
        "Override data directory (datastores, persistent state)."])
    var dataDir: String? = null

    /** Override the cache directory. */
    @Option(names = ["-x", "--cache-dir"], paramLabel = "PATH", description = [
        "Override cache directory (temporary files, caches)."])
    var cacheDir: String? = null

    /** Override the run directory (PID, sockets, etc.). */
    @Option(names = ["-r", "--run-dir"], paramLabel = "PATH", description = [
        "Override run directory (PID, sockets, runtime files)."])
    var runDir: String? = null

    /** Choose service mode explicitly. */
    @Option(names = ["-m", "--service-mode"], paramLabel = "MODE", description = [
    ...
    var modeShortcuts: ModeShortcuts? = null

    class ModeShortcuts {
        @Option(names = ["--service", "--daemon"], description = ["Shortcut for --service-mode=service"]) var service: Boolean = false
        @Option(names = ["--user", "--app"], description = ["Shortcut for --service-mode=user"]) var user: Boolean = false
    }
    ...
    fun directoryOverrides(): MutableMap<String, String> = mutableMapOf<String, String>().apply {
