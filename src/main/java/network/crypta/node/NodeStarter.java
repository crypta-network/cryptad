...
public class NodeStarter implements WrapperListener {
    ...
    public static RandomSource globalTestInit(
        String testName, boolean enablePlug, LogLevel logThreshold,
        String details, boolean noDNS)
     ...
     *                      automatically if it does not exist. You should use the same one in
     *                      {@link TestNodeParameters#baseDirectory} afterwards for each individual test
     *                      node as long as it has a distinct port. See its JavaDoc.<br> The function
     *                      will NOT fail if the directory exists already. You should make sure on your
     *                      own to delete this before and after tests to ensure a clean state. Notice
     *                      that JUnit provides a mechanism for automatic creation and deletion of test
     *                      directories (TemporaryFolder).
     * @param RandomSource  The random number generator of the Node. Null for the default of
     *                      {@link Yarrow}.
     *                      <br>You might want to use a {@link DummyRandomSource} in unit tests:<br>
     *                      - Unlike Yarrow, it won't block startup waiting for entropy.<br> - It allows
     *                      you to specify a seed which you then can print to stdout so randomized unit
     *                      tests are reproducible.<br> - It should be a lot faster than Yarrow.<br>
     *
    ...
    public static Node createTestNode(
        int port, int opennetPort, String testName, boolean disableProbabilisticHTLs, short maxHTL,
        int dropProb, RandomSource random, Executor executor, int threadLimit, long storeSize,
        boolean ramStore, boolean enableSwapping, boolean enableARKs, boolean enableULPRs,
        boolean enablePerNodeFailureTables, boolean enableSwapQueueing, boolean enablePacketCoalescing,
        int outputBandwidthLimit, boolean enableFOAF, boolean connectToSeednodes, boolean longPingTimes,
        boolean useSlashdotCache, String ipAddressOverride) throws NodeInitException {
        return createTestNode(port, opennetPort, testName, disableProbabilisticHTLs, maxHTL, dropProb,
                              random, executor, threadLimit, storeSize, ramStore, enableSwapping,
                              enableARKs, enableULPRs, enablePerNodeFailureTables, enableSwapQueueing,
                              enablePacketCoalescing, outputBandwidthLimit, enableFOAF,
                              connectToSeednodes, longPingTimes, useSlashdotCache, ipAddressOverride,
                              false);
    }
    ...
    public static Node createTestNode(
        ...
        boolean enablePerNodeFailureTables, boolean enableSwapQueueing, boolean enablePacketCoalescing,
        int outputBandwidthLimit, boolean enableFOAF, boolean connectToSeednodes, boolean longPingTimes,
        boolean useSlashdotCache, String ipAddressOverride, boolean enableFCP) throws NodeInitException {
    ...
    /**
     * check whether the OS, JVM and wrapper are 64bits On Windows this will be always true (the wrapper
     * we deploy is 32bits)
     */
    ...
    /**
     * The start method is called when the WrapperManager is signaled by the native wrapper code that it
     * can start its application.  This method call is expected to return, so a new thread should be
     * launched if necessary.
     *
     * @param args List of arguments used to initialize the application.
     *
     * @return Any error code if the application should exit on completion of the start method.  If there
     * were no problems then this method should return null.
     */
    @Override
    public Integer start(String[] args) {
        ...
        NativeThread plug = new NativeThread(useless, "Plug", NativeThread.PriorityLevel.MAX_PRIORITY.value, false);
        // Not daemon, but doesn't do anything.
        // Keeps the JVM alive.
        plug.setDaemon(false);
        plug.start();
     ...
     *
     * @return The exit code to actually return to the OS.  In most cases, this should just be the value
     * of exitCode, however the user code has the option of changing the exit code if there are any
    ...
    public static final class TestNodeParameters {
        ...
        /**
         * The directory where the test node will put all its data. <br> Will be created automatically if
         * it does not exist.<br> {@link NodeStarter#createTestNode(TestNodeParameters)} will NOT fail if
         * this exists. You should make sure on your own to delete this before and after tests to ensure
         * a clean state. Notice that JUnit provides a mechanism for automatic creation and deletion of
         * test directories (TemporaryFolder).<br> Notice that a subdirectory with the name being the
         * port number of the node will be created there, and all data of the node will be put into it.
         * So you can and should use the same baseDirectory when calling
