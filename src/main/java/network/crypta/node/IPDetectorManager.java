...
/**
 * Coordinates IP-detection and port-forwarding detectors and decides when to run them.
 *
 * <p>The manager aggregates {@link ExternalIpDetector} implementations (typically one, but multiple
 * are allowed) and applies lightweight heuristics to determine when detection should be executed.
 * It also receives callbacks from {@link PortForwardProvider} detectors and updates user alerts
 * accordingly. Long-running work executes on the node's executor; this class keeps only the
 ...
 */
public class IPDetectorManager implements ForwardPortCallback {
  private static final Logger LOG = LoggerFactory.getLogger(IPDetectorManager.class);
    ...
    public HTMLNode getHTMLText() {
      ...
      if (portsNotForwarded.length == 1) {
        NodeL10n.getBase()
            .addL10nSubstitution(
                div,
                "IPDetectorManager.forwardPort" + keySuffix,
        ...
        NodeL10n.getBase()
            .addL10nSubstitution(
                div,
                "IPDetectorManager.forwardTwoPorts" + keySuffix,
      ...
      if (suggestPortForward) {
        if (portsNotForwarded.length == 1) {
          NodeL10n.getBase()
              .addL10nSubstitution(
                  div,
                  "IPDetectorManager.suggestForwardPortWithLink",
          ...
          NodeL10n.getBase()
              .addL10nSubstitution(
                  div,
                  "IPDetectorManager.suggestForwardTwoPortsWithLink",
  ...
  ExternalIpDetector[] detectors;
  PortForwardProvider[] portForwardProviders;
  ...
  IPDetectorManager(Node node, NodeIPDetector detector) {
    detectors = new ExternalIpDetector[0];
    portForwardProviders = new PortForwardProvider[0];
    this.node = node;
  ...
  private static final String L10N_PREFIX = "IPDetectorManager.";
  ...
  /**
   * Start the provider detection, if necessary. Either way, schedule another attempt in 1 minute's
   * time.
   */
  @SuppressWarnings("java:S1181") // External detector boundary: contain provider-caused Errors
  ...
  /**
   * Registers an IP-detection provider.
   *
   * @param d provider instance to register
   * @throws NullPointerException if {@code d} is {@code null}
   */
  public void registerExternalDetector(ExternalIpDetector d) {
    if (d == null) throw new NullPointerException();
    synchronized (this) {
      lastDetectAttemptEndedTime = -1;
      detectors = Arrays.copyOf(detectors, detectors.length + 1);
      detectors[detectors.length - 1] = d;
    }
    if (LOG.isDebugEnabled()) LOG.debug("Register IP detector provider: {}", d);
    maybeRun();
  }

  /**
   * Unregisters a previously registered IP-detection provider.
   *
   * @param d provider instance to remove
   */
  public void unregisterExternalDetector(ExternalIpDetector d) {
    DetectorRunner runningDetector;
    synchronized (this) {
      int count = 0;
      for (ExternalIpDetector provider : detectors) {
        if (provider == d) count++;
      }
      if (count == 0) return;
      ExternalIpDetector[] newDetectors = new ExternalIpDetector[detectors.length - count];
      int x = 0;
      for (ExternalIpDetector provider : detectors) {
        if (provider != d) newDetectors[x++] = provider;
      }
  ...
  private final HashMap<ExternalIpDetector, DetectorRunner> runners = new HashMap<>();
  private final HashSet<ExternalIpDetector> failedRunners = new HashSet<>();
  private long lastDetectAttemptEndedTime;
  private volatile long firstTimeUrgent;

  /**
   * Evaluates the current state and runs detection detectors when heuristics allow.
   *
  ...
  public void maybeRun() {
    if (!started) return;
    if (LOG.isDebugEnabled()) LOG.debug("Maybe running IP detection detectors");
    ...
    if (switch (gateOnDetectorState(now)) {
  ...
  private GateDecision gateOnDetectorState(long now) {
    synchronized (this) {
      if (detectors.length == 0) {
        if (LOG.isDebugEnabled()) LOG.debug("No IP detection detectors registered");
        detector.markExternalDetectionsComplete();
        return GateDecision.RETURN;
      }
      if (runners.size() == detectors.length) {
        if (LOG.isDebugEnabled()) LOG.debug("All IP detection detectors already running");
        return GateDecision.RETURN;
      }
      if (failedRunners.size() == detectors.length) {
  ...
  /**
   * Given that we have no peers, should we run the detection detectors? Algorithm: Run the
   * detection once every 6 hours.
   *
  ...
  /**
   * Given that we have some peers, should we run the detection detectors?
   *
  ...
  /**
   * Should we run the detection detectors despite having a directly detected IP address?
   *
   * @param now The time at the beginning of the calling method.
   * @param peers The node's peers.
  ...
  private void startDetect() {
    ...
    synchronized (this) {
      failedRunners.clear();
      for (ExternalIpDetector provider : detectors) {
        if (runners.containsKey(provider)) continue;
        DetectorRunner d = new DetectorRunner(provider);
        runners.put(provider, d);
        node.network().executor().execute(d, "IP detector runner for " + provider.getClass());
      }
    }
  }

  /**
   * Executes detection for a single provider instance. Runs on the node executor and isolates
   * provider failures so a misbehaving provider cannot crash the manager.
   */
  public class DetectorRunner implements Runnable {

    final ExternalIpDetector provider;

    public DetectorRunner(ExternalIpDetector detector) {
      provider = detector;
    }

    /** Stops the underlying detector provider instance directly. */
    public void kill() {
      provider.terminate();
    }
    ...
    /** Runs the provider, updates alerts, and notifies the node detector. */
    public void realRun() {
      if (LOG.isDebugEnabled()) LOG.debug("Run provider detection");
      try {
        List<DetectedIP> v = collectDetectedIPs();
        synchronized (IPDetectorManager.this) {
        ...
        boolean finished;
        synchronized (IPDetectorManager.this) {
          runners.remove(provider);
          finished = runners.isEmpty();
        }
        if (finished) detector.markExternalDetectionsComplete();
      }
    }

    /** Collects the IPs reported by the provider; shields provider exceptions. */
    @SuppressWarnings("java:S1181") // External detector boundary: contain provider-caused Errors
    private List<DetectedIP> collectDetectedIPs() {
      List<DetectedIP> result = new ArrayList<>();
      try {
        DetectedIP[] detected = provider.getAddress();
    ...
    private boolean hasAnyValidPublicAddress(List<DetectedIP> v) {
      for (DetectedIP ip : v) {
        if (LOG.isDebugEnabled())
          LOG.debug(LOG_DETECTED_IP_PREFIX + "{}" + FOR_WORD + "{}", ip, provider);
  ...
  public void registerPortForwardProvider(PortForwardProvider forward) {
    ...
    synchronized (this) {
      portForwardProviders = Arrays.copyOf(portForwardProviders, portForwardProviders.length + 1);
      portForwardProviders[portForwardProviders.length - 1] = forward;
    }
    if (LOG.isDebugEnabled()) LOG.debug("Register port forward provider: {}", forward);
  ...
  public void unregisterPortForwardProvider(PortForwardProvider forward) {
    synchronized (this) {
      int count = 0;
      for (PortForwardProvider portForwardProvider : portForwardProviders) {
        if (portForwardProvider == forward) count++;
      }
      if (count == 0) return;
      PortForwardProvider[] newProviders =
          new PortForwardProvider[portForwardProviders.length - count];
      int x = 0;
      for (PortForwardProvider portForwardProvider : portForwardProviders) {
        if (portForwardProvider != forward) newProviders[x++] = portForwardProvider;
      }
      portForwardProviders = newProviders;
    }
  }

  @SuppressWarnings("java:S1181") // External detector boundary: contain provider-caused Errors
  void notifyPortChange(final Set<ForwardPort> newPorts) {
    PortForwardProvider[] localPortForwardProviders;
    synchronized (this) {
      localPortForwardProviders = portForwardProviders;
    }
    for (final PortForwardProvider provider : localPortForwardProviders) {
      node.network()
          .executor()
          .execute(
              () -> {
                try {
                  provider.onChangePublicPorts(newPorts, IPDetectorManager.this);
                } catch (Throwable t) {
                  LOG.error("onChangePublicPorts on {} threw: {}", provider, t, t);
                }
              },
              "Notify " + provider + " of ports list change");
    }
  }

  /**
   * Receives results from port-forwarding detectors and logs a summarized status per port.
   *
  ...
  /**
   * Returns whether at least one IP-detection provider is registered.
   *
  ...
  /**
   * Returns whether a legacy JSTUN detector is loaded or in the process of loading.
   *
   * <p>The JSTUN integration path is no longer available, so this always returns {@code false}.
   *
