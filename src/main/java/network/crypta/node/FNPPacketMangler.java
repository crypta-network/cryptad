...
public class FNPPacketMangler implements OutgoingPacketMangler {
	...
	*/
	...
	private void processJFKMessage1(byte[] payload,int offset,PeerNode pn,Peer replyTo, boolean unknownInitiator, int setupType, int negType)
	{
		...
		if(unknownInitiator) {
			...
			if(!MessageDigest.isEqual(expectedIdentityHash, crypto.getIdentityHash())) {
				Logger.error(this, "Invalid unknown-initiator JFK(1), IDr' is " +
                                   HexUtil.bytesToHex(expectedIdentityHash) + " should be " +
	...
	private void handleNoContextsException(NoContextsException e,
			FNPPacketMangler.NoContextsException.CONTEXT context) {
	...
	private void processJFKMessage3(byte[] payload, int inputOffset, PeerNode pn,Peer replyTo, boolean oldOpennetPeer, boolean unknownInitiator, int setupType, int negType)
	{
		...
		if(message4 != null) {
			...
			if(logDEBUG) Logger.debug(this, "No message4 found for " +
                                            HexUtil.bytesToHex(authenticator) + " responderExponential " + Fields.hashCode(responderExponential) + " initiatorExponential " + Fields.hashCode(initiatorExponential) + " nonceResponder " + Fields.hashCode(nonceResponder) + " nonceInitiator " + Fields.hashCode(nonceInitiatorHashed) + " address " +
		...
		if(logDEBUG) Logger.debug(this, "The shared Master secret is : " +
                                        HexUtil.bytesToHex(computedExponential) + " for " + pn);
		...
		if((!dontWant) && !crypto.allowConnection(pn, replyTo.getFreenetAddress())) {
			if(pn instanceof DarknetPeerNode peerNode) {
				...
				System.out.println("Disconnecting permanently from your friend \""+peerNode.getName()+"\" because other peers are using the same IP address!");
			}
	...
	private ECDHLightContext _genECDHLightContext() {
        ...
        if(logDEBUG) Logger.debug(this, "ECDSA Signature: " + HexUtil.bytesToHex(ctx.ecdsaSig) + " for " +
