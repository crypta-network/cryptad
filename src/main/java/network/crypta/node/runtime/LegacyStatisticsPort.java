...
final class LegacyStatisticsPort implements StatisticsPort {

  private static final NumberFormat THOUSAND_POINT = NumberFormat.getInstance();
  ...
  private static final class PeerStatusSummary {
    ...
    private PeerStatusSummary(PeerNodeStatus[] peerNodeStatuses, PeerStatusCounts counts) {
      this.peerNodeStatuses = peerNodeStatuses;
      this.counts = counts;
    }

    private boolean hasConnectedOrBackedOffPeers() {
  ...
  LegacyStatisticsPort(Node node, NodeClientCore core) {
    this.node = node;
    this.core = core;
  ...
  @Override
  public synchronized StatisticsPageSnapshot overview(boolean advancedMode) {
    refreshBandwidthStats();

    HTMLNode contentNode = new HTMLNode("#");
    PeerStatusSummary peerStatusSummary = preparePeerStatusSummary();
    renderStatisticsPage(advancedMode, contentNode, peerStatusSummary);

    return new StatisticsPageSnapshot(
        contentNode.generate(),
        node.isUsingWrapper(),
        node.getConfig().get("logger").getBoolean("enabled"));
  }

  @Override
  public synchronized StatisticsPageSnapshot requesters() {
    HTMLNode contentNode = new HTMLNode("#");
    drawClientRequestersBox(contentNode);
    return new StatisticsPageSnapshot(contentNode.generate(), false, false);
  }
  ...
  private void refreshBandwidthStats() {
    try {
      ...
      // Keep the legacy statistics snapshot best-effort.
    }
  }
  ...
  private void renderStatisticsPage(
      boolean advancedMode, HTMLNode contentNode, PeerStatusSummary peerStatusSummary) {
    ...
    addCoreInfoboxes(advancedMode, nodeUptimeSeconds, leftColumn, peerStatusSummary, contentNode);
  ...
  private void addCoreInfoboxes(
    ...
    leftColumn.addChild("%", STAT_GATHERING_BOX_PLACEHOLDER);
  ...
  private void drawRejectReasonsBox(HTMLNode nextTableCell, boolean local) {
    ...
    boolean success =
        local
            ? NodeStatsHtmlRenderer.getLocalRejectReasonsTable(stats, rejectReasonsTable)
            : NodeStatsHtmlRenderer.getRejectReasonsTable(stats, rejectReasonsTable);
    if (!success) {
      return;
    }
  ...
  private void drawJVMStatsBox(HTMLNode jvmStatsInfobox, boolean advancedModeEnabled) {
    ...
    jvmStatsList.addChild(
        ...
        l10n(
            ...
            new String[] {
              THOUSAND_POINT.format(stats.getActiveThreadCount()),
              Integer.toString(stats.getThreadLimit())
            }));
    jvmStatsList.addChild(
        "li", l10n("cpus", COUNT_LABEL, Integer.toString(runtime.availableProcessors())));
    jvmStatsList.addChild(
  ...
  private void drawClientRequestersBox(HTMLNode box) {
    ...
    for (ClientRequester request : requests) {
      ...
      try {
        ...
        if (requestString.indexOf(':') > requestString.indexOf('@')) {
          requestString = requestString.substring(0, requestString.indexOf(':'));
        }
        row.addChild("td", requestString);
      } catch (Exception _) {
  ...
  private void addStoreStatsRow(
    ...
    row.addChild(
        "td",
        THOUSAND_POINT.format(sessionAccess.falsePos())
  ...
  private long getTotalUptimeSeconds(StoreAccessStats totalAccess) {
    ...
    return core.getClientLayerPersister()
        .getBandwidthStatsPutter()
        .getLatestUptimeData()
        .getTotalUptime();
  }
  ...
  private String formatTotalValue(StoreAccessStats totalAccess, long totalValue) {
    ...
    return NBSP_OPEN_PAREN + THOUSAND_POINT.format(totalValue) + ")";
  }
  ...
  private void drawUnclaimedFifoMessageCountsBox(HTMLNode unclaimedFifoMessageCountsInfobox) {
    ...
    int i = 0;
    int totalCount = 0;
    for (Map.Entry<String, Integer> entry : unclaimedFifoMessageCountsMap.entrySet()) {
    ...
    if (advancedModeEnabled && activityList != null) {
      ...
      activityList.addChild(
          ...
          "BackgroundFetcherByUSKSize:\u00a0"
              + core.getUskManager().getBackgroundFetcherByUSKSize());
      activityList.addChild(
          "li",
          "temporaryBackgroundFetchersLRUSize:\u00a0"
              + core.getUskManager().getTemporaryBackgroundFetchersLRU());
      activityList.addChild(
          "li",
          "outputBandwidthLiabilityUsage:\u00a0"
              + fix3p1pct.format(node.network().stats().getBandwidthLiabilityUsage()));
    }
  }
  ...
  private void drawBandwidth(
    ...
    BandwidthStatsContainer bandwidthStats =
        core.getClientLayerPersister().getBandwidthStatsPutter().getLatestBWData();
  ...
  private HTMLNode drawActivity(HTMLNode activityInfoboxContent, Node node) {
  ...
  private void addNodeCircle(HTMLNode circleTable, double myLocation) {
    ...
    nodeCircleInfoboxContent.addChild(
        ...
        "+");
    Object[] knownLocsCopy = stats.getKnownLocations(-1);
    ...
    for (int i = 0; i < locations.length; i++) {
      ...
      int histogramIndex = (int) Math.floor(location * HISTOGRAM_LENGTH);
    ...
    int histogramDiv = simpleHistogramDivisor(histogram);
    for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
      ...
      double histogramPercent = (double) histogram[i] / histogramDiv;
  ...
  private void populatePeerCircleData(
    ...
    for (PeerNodeStatus peerNodeStatus : peerNodeStatuses) {
      ...
      histogramIndex %= HISTOGRAM_LENGTH;
  ...
  private void addPeerCircle(
    ...
    for (int i = 0; i < HISTOGRAM_LENGTH; i++) {
      ...
      HTMLNode peerHistogramGraphCell = createHistogramGraphCell(peerHistogramGraphTableRow);
      double histogramPercent = ((double) histogramConnected[i]) / histogramDiv;
