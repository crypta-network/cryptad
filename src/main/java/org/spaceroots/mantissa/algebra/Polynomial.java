 ...
 *
 * <p>This is an abstract class that only declares general methods but does not hold the
 * coefficients by themselves. Specific subclasses are used to handle exact rational coefficients or
 * approximate real coefficients. This design is taken from the various java.awt.geom classes
 * (Point2D, Rectangle2D ...)
 *
 * <p>The methods implemented deal mainly with the polynomials algebra (addition, multiplication
 * ...) but the analysis aspects are also considered (value of the polynom for a given unknown,
 * derivative).
 *
...
public abstract class Polynomial implements Serializable {
  ...
  /**
   * Get the derivative of the instance with respect to the unknown. The derivative of a n degree
   * polynomial is a n-1 degree polynomial of the same type.
   *
   ...
   *
   * <p>In addition to classical algebra operations, euclidian division and remainder are handled.
   */
  ...
  /**
   * This class stores the result of the euclidian division of two polynomials. This class is a
   * simple placeholder, it does not provide any processing method
   *
