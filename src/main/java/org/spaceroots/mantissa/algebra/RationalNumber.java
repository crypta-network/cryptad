...
public class RationalNumber implements Serializable {
  ...
  /**
   * Check if the instance is equal to another rational number. Equality here is having the same
   * value.
   *
   * @return true if the object is a rational number which has the same value as the instance
   */
  ...
  /**
   * Returns a hash code value for the object. The hash code value is computed from the reduced
   * numerator and denominator, hence equal rational numbers have the same hash code, as required by
  ...
  /**
   * Returns a string representation of the rational number. The representation is reduced: there is
   * no common factor left between the numerator and the denominator. The '/' character and the
   * denominator are displayed only if the denominator is not one. The sign is on the numerator.
   *
