...
/**
 * The FIPS PUB 180-2 standard specifies four secure hash algorithms (SHA-1, SHA-256, SHA-384 and
 * SHA-512) for computing a condensed representation of electronic data (message). When a message of
 * any length < 2^^64 bits (for SHA-1 and SHA-256) or < 2^^128 bits (for SHA-384 and SHA-512) is
 * input to an algorithm, the result is an output called a message digest. The message digests range
 * in length from 160 to 512 bits, depending on the algorithm. Secure hash algorithms are typically
 * used with other cryptographic algorithms, such as digital signature algorithms and keyed-hash
 * message authentication codes, or in the generation of random numbers (bits).
 *
 * <p>The four hash algorithms specified in this "SHS" standard are called secure because, for a
 * given algorithm, it is computationally infeasible 1) to find a message that corresponds to a
 * given message digest, or 2) to find two different messages that produce the same message digest.
 * Any change to a message will, with a very high probability, result in a different message digest.
 * This will result in a verification failure when the secure hash algorithm is used with a digital
 * signature algorithm or a keyed-hash message authentication algorithm.
 *
 * <p>A "SHS change notice" adds a SHA-224 algorithm for interoperability, which, like SHA-1 and
 * SHA-256, operates on 512-bit blocks and 32-bit words, but truncates the final digest and uses
 * distinct initialization values.
 *
 * <p><b>References:</b>
 *
 * <ol>
 *   <li>NIST FIPS PUB 180-2, "Secure Hash Signature Standard (SHS) with change notice", National
 *       Institute of Standards and Technology (NIST), 2002 August 1, and U.S. Department of
 *       Commerce, August 26.<br>
 *       <a href="http://csrc.ncsl.nist.gov/CryptoToolkit/Hash.html">
 *       http://csrc.ncsl.nist.gov/CryptoToolkit/Hash.html</a>
 *   <li>NIST FIPS PUB 180-1, "Secure Hash Standard", U.S. Department of Commerce, May 1993.<br>
 *       <a href="http://www.itl.nist.gov/div897/pubs/fip180-1.htm">
 *       http://www.itl.nist.gov/div897/pubs/fip180-1.htm</a>
 *   <li>Bruce Schneier, "Section 18.7 Secure Hash Algorithm (SHA)", <cite>Applied Cryptography, 2nd
 *       edition</cite>, <br>
...
public final class Sha1 extends MessageDigest implements Cloneable {
  ...
  /** Private contextual byte count, sent in the next block, after the ending padding block. */
  private long bytes;
   ...
   * final hash and returns the final value as a byte[20] array. Once engineDigest has been called,
   ...
   * engineDigest(byte[], int offset, int len).
   *
   * <p>Overrides the protected abstract method of java.security.MessageDigestSpi.
   *
   ...
   * @return the length of the digest stored in the output buffer.
   */
  public int engineDigest(byte[] hashvalue, int offset, final int len) throws DigestException {
    if (len >= HASH_LENGTH) {
