...
/**
 * Tiger is a fast new hash function, by Ross Anderson and Eli Biham. This class is a simple Java
 * implementation of the 192-bit Tiger algorithm, based on the sample C code published by Eli Biham
 * on http://www.cs.technion.ac.il/~biham/Reports/Tiger/ in its HTML reference Appendix.
 *
 * <p>It computes a 192-bit digest that is considered stronger, but faster to compute than 160-bit
 * SHA-1. Its input is a set of 64-bytes blocks. The last block of a digested message must include a
 * required padding byte which must be 0x01, the remaining padding bytes must be set to 0x00.
 *
 * <p>May be in the future, this class will be part of the standard JCE (Java Cryptography
 * Environment) included in Java 1.5, code named... Tiger! For now Java 1.4, code named Merlin, does
 * not have this digest algorithm, and only includes Adler32 or CRC32 (in java.util.zip), and MD4,
 * MD5 or SHA1 (in javax.crypto.Mac, included in the SUN JCE).
 */
public final class Tiger extends MessageDigest implements Cloneable {
  ...
  /**
   * Private context for incomplete blocks and padding bytes. INVARIANT: padding must be in 0..63.
   * When the padding reaches 64, a new block is computed, and the 56 last bytes are kept in the
   * padding history.
   */
  private byte[] pad;
  ...
  /** Private Contextual byte count, send in the next block, after the ending padding block. */
  private long bytes;
   ...
   *
   * <p>Can be used to allocate your own output buffer when computing multiple digests.
   *
   * <p>Overrides the protected abstract method of java.security.MessageDigestSpi.
   *
   ...
   *
   * <p>Overrides the protected abstract method of <code>java.security.MessageDigestSpi</code>.
   */
   ...
   *
   * <p>Input length can be any size. May require internal buffering, if input blocks are not
  ...
  /**
   * Completes the hash computation by performing final operations such as padding. Computes the
   * final hash and returns the final value as a byte[24] array. Once engineDigest has been called,
   * the engine will be automatically reset as specified in the JavaSecurity MessageDigest
   * specification.
   *
   * <p>For faster operations with multiple digests, allocate your own array and use
   * engineDigest(byte[], int offset, int len). * Overrides the protected abstract method of
   ...
   * engineDigest has been called, the engine will be automatically reset (see engineReset).
   *
   * <p>Overrides the protected abstract method of java.security.MessageDigestSpi.
   *
   * @param hashvalue the output buffer in which to store the digest.
   * @param offset offset to start from in the output buffer
   * @param len number of bytes within buf allotted for the digest. Both this default implementation
   *     and the SUN provider do not return partial digests. The presence of this parameter is
   *     solely for consistency in our API's. If the value of this parameter is less than the actual
   *     digest length, the method will throw a DigestException. This parameter is ignored if its
   *     value is greater than or equal to the actual digest length.
   * @return the length of the digest stored in the output buffer.
   */
  ...
  /**
   * Updates the digest using the specified array of bytes, starting at the specified offset, but an
   * implied length of exactly 64 bytes.
   *
   * <p>Requires no internal buffering, but assumes a fixed input size, in which the required
   * padding bytes may have been added.
   *
   * @param input the array of bytes to use for the update.
   * @param offset the offset to start from in the array of bytes.
   */
  private void computeBlock(final byte[] input, int offset) {
            ...
            ^ (w1 =
                ((long)
                    ...
                    | ((long)
                        ...
                        << 32));
    b =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + hB) * 5
                - (S0[(lo = (int) a) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (a >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w2 =
                ((long)
                    ...
                    | ((long)
                        ...
                        << 32));
    c =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + c) * 5
                - (S0[(lo = (int) b) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (b >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w3 =
                ((long)
                    ...
                    | ((long)
                        ...
                        << 32));
    a =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + a) * 5
                - (S0[(lo = (int) c) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (c >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w4 =
                ((long)
                    ...
                    | ((long)
                        ...
                        << 32));
    b =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + b) * 5
                - (S0[(lo = (int) a) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (a >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
    ...
    b =
        ((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + b) * 5
            - (S0[(lo = (int) a) & 0xff]
                ...
                ^ S2[(hi = (int) (a >>> 32)) & 0xff]
                ^ S3[(hi >>> 16) & 0xff]);
    c = ((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + c) * 5;
    /* Start scheduling the current input set before next pass. */
    w7 ^=
        w6 -=
            (w5 += w4 ^= w3 -= (w2 += w1 ^= w0 -= w7 ^ 0xa5a5a5a5a5a5a5a5L) ^ ((~w1) << 19))
                ^ ((~w4) >>> 23);
    /* Second pass on scheduled input, with multiplier equal to 7. */
    c =
        (c
                - (S0[(lo = (int) (b ^= w0 += w7)) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (b >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w1 -= (w0) ^ ((~w7) << 19));
    a =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + a) * 7
                - (S0[(lo = (int) c) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (c >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w2 ^= w1);
    b =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + b) * 7
                - (S0[(lo = (int) a) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (a >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w3 += w2);
    c =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + c) * 7
                - (S0[(lo = (int) b) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (b >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
    ...
    a =
        ((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + a) * 7
            - (S0[(lo = (int) c) & 0xff]
                ...
                ^ S2[(hi = (int) (c >>> 32)) & 0xff]
                ^ S3[(hi >>> 16) & 0xff]);
    b = ((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + b) * 7;
    /* Start scheduling the current input set before next pass. */
    w7 ^=
        w6 -=
            (w5 += w4 ^= w3 -= (w2 += w1 ^= w0 -= w7 ^ 0xa5a5a5a5a5a5a5a5L) ^ ((~w1) << 19))
    ...
    b =
        (b
                - (S0[(lo = (int) (a ^= w0 += w7)) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (a >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w1 -= w0 ^ ((~w7) << 19));
    c =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + c) * 9
                - (S0[(lo = (int) b) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (b >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w2 ^= w1);
    a =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + a) * 9
                - (S0[(lo = (int) c) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (c >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
            ^ (w3 += w2);
    b =
        (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + b) * 9
                - (S0[(lo = (int) a) & 0xff]
                    ^ S1[(lo >>> 16) & 0xff]
                    ^ S2[(hi = (int) (a >>> 32)) & 0xff]
                    ^ S3[(hi >>> 16) & 0xff]))
    ...
    hB =
        (b =
                (((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24])
                                + b)
                            * 9
                        - (S0[(lo = (int) a) & 0xff]
                            ^ S1[(lo >>> 16) & 0xff]
    ...
    hC +=
        ((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + c) * 9
            - (S0[(lo = (int) b) & 0xff]
                ...
                ^ S2[(hi = (int) (b >>> 32)) & 0xff]
                ^ S3[(hi >>> 16) & 0xff]);
    hA ^= ((S3[(lo >>> 8) & 0xff] ^ S2[lo >>> 24] ^ S1[(hi >>> 8) & 0xff] ^ S0[hi >>> 24]) + a) * 9;
  }

  /**
   * Precomputed constant Tiger "S-Boxes" in 64-bit format. Each of these four table have 256
   * elements.
   */
