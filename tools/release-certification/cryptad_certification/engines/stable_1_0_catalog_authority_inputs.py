...
MAX_SUBJECT_BUNDLE_BYTES = 256 * 1024 * 1024
MAX_EVIDENCE_BYTES = MAX_SUBJECT_BUNDLE_BYTES + 32 * 1024 * 1024
PRIMARY_SUBJECT_BUNDLE = "stable-1.0-primary-subject-bundle.zip"
            ...
            for index, coordinate in enumerate(coordinates["artifacts"]):
                ...
                with zipfile.ZipFile(archive_path) as archive:
                    ...
                    for selection in members:
                        ...
                        digest = hashlib.sha256()
                        written = 0
                        with archive.open(member) as source_stream, destination.open(
                            "wb"
                        ) as sink:
                            for chunk in iter(
                                ...
                                written += len(chunk)
                                if written > member.file_size or written > member_limit:
                                    raise AssemblyError(
                                        "catalog-authority-input-member-size-invalid"
                                    )
                                digest.update(chunk)
                                sink.write(chunk)
                        if (
