 ...
 *
 * <ol>
 *   <li>Create a context with explicit limits using the main constructor, or start from a caller's
 *       chosen library defaults.
 ...
 * bounded by the constants declared on this type.
 *
 * <p>Notable behaviors:
 *
 * <ul>
...
public final class FetchContext implements Serializable {
  ...
  /**
   * Default cooldown duration in milliseconds applied after repeated retries.
   *
   * <p>This mirrors the historical request-scheduler contract while keeping {@code FetchContext}
   * compile-neutral and usable from leaf modules.
   */
  public static final long DEFAULT_COOLDOWN_PERIOD_MILLIS = 30L * 60L * 1000L;

  /**
   * Default maximum retry count before cooldown is entered.
   *
   * <p>This mirrors the historical request-scheduler contract while keeping {@code FetchContext}
   * compile-neutral and usable from leaf modules.
   */
  public static final int DEFAULT_COOLDOWN_RETRIES = 3;

  /**
   * Maximum total data-plus-check blocks permitted per segment.
   *
   * <p>This mirrors the long-standing FEC segment sizing contract while keeping {@code
   * FetchContext} independent of the runtime-owned codec implementation classes.
   */
  ...
  /**
   * Number of attempts before we go into cooldown. Must be less than or equal to {@link
   * #DEFAULT_COOLDOWN_RETRIES}.
   */
  private int cooldownRetries;

  /**
   * Time period for which we go into cooldown. Must be NO LESS THAN {@link
   * #DEFAULT_COOLDOWN_PERIOD_MILLIS}, because ULPRs will ensure rapid success with that interval or
   * less.
   */
  ...
  public FetchContext(FetchContextOptions options) {
    ...
    if (maxDataBlocksPerSegment < 0 || maxDataBlocksPerSegment > MAX_TOTAL_BLOCKS_PER_SEGMENT)
    ...
    if (maxCheckBlocksPerSegment < 0 || maxCheckBlocksPerSegment > MAX_TOTAL_BLOCKS_PER_SEGMENT)
      throw new IllegalArgumentException("Bad max blocks per segment");
    ...
    this.cooldownRetries = DEFAULT_COOLDOWN_RETRIES;
    this.cooldownTime = DEFAULT_COOLDOWN_PERIOD_MILLIS;
   ...
   *     #DEFAULT_COOLDOWN_RETRIES} inclusive.
  ...
  public void setCooldownRetries(int cooldownRetries) {
    ...
    if (cooldownRetries > DEFAULT_COOLDOWN_RETRIES)
   ...
   *     not less than {@link #DEFAULT_COOLDOWN_PERIOD_MILLIS}.
  ...
  public void setCooldownTime(long cooldownTime, boolean force) {
    ...
    if (cooldownTime < DEFAULT_COOLDOWN_PERIOD_MILLIS && !force)
      throw new IllegalArgumentException(
          "Invalid COOLDOWN_PERIOD: Must be >= "
              + DEFAULT_COOLDOWN_PERIOD_MILLIS
  ...
  public FetchContext(DataInputStream dis) throws StorageFormatException, IOException {
    ...
    if (dataBlocks < 0 || dataBlocks > MAX_TOTAL_BLOCKS_PER_SEGMENT)
    ...
    if (checkBlocks < 0 || checkBlocks > MAX_TOTAL_BLOCKS_PER_SEGMENT)
      throw new StorageFormatException("Bad max blocks per segment");
