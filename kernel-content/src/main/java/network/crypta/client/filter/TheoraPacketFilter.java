 ...
 * (Identification → Comment → Setup) before passing through later frame packets unchanged.
 *
 ...
 * intact. Typical usage is to feed incoming packets in order; the filter maintains a minimal
 * internal state to track which header is expected next, and after all three headers are accepted,
 * it treats later packets as video frames and returns them unchanged.
 *
 ...
 *   <li>Checks identification fields and set up tables for basic bounds.
...
public class TheoraPacketFilter implements CodecPacketFilter {
  ...
  /**
   * Constructs a new filter with the initial state expecting the Theora Identification header. The
   * instance maintains a minimal state only to track which header should arrive next; it does not
   ...
   * followed by the Comment and Setup headers, then treats later packets as frames.
   *
