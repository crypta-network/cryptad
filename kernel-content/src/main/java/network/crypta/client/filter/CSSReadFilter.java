...
public class CSSReadFilter implements ContentDataFilter, CharsetExtractor {
   ...
   * designed to operate on streaming inputs and must not rely on {@code available()} to detect the
   * end of stream. When the provided {@code charset} is unsupported, an {@link
   * UnknownCharsetException} is thrown. Structural or policy violations are reported as {@link
   * DataFilterException} subtypes.
   *
   ...
   *
   * @param input the byte stream containing the original stylesheet; may be networking- or
   *     pipe-backed and should be consumed in a streaming fashion without assuming the total size
   *     in advance.
   ...
   * @throws IOException if decoding of the prefix fails due to an unknown or unsupported charset,
   *     or if a malformed declaration prevents further inspection.
   */
  @Override
