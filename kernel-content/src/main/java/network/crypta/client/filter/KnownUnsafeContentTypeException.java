 ...
 * execute scripts or load external resources. The exception is used when there is no applicable
 * filter that can safely sanitize the response. Callers typically catch it at boundaries that
 * translate errors to client‑visible responses, such as HTTP handlers. They then present a
 * descriptive error message that explains the risks and recommended next steps.
 *
 * <p>The instance carries the {@code FilterMIMEType} that triggered the decision. The exception is
 * immutable and thread‑safe for read‑only use after construction. There are no retry semantics: the
