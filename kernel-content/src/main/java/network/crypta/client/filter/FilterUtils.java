 ...
 * thread-safe.
 *
 * <p>Typical usage is to call individual predicates before accepting or acting upon untrusted input
 * (for example, when parsing style attributes, filter expressions, or plugin-provided
...
public class FilterUtils {
  ...
  /**
   * Validates a CSS/SVG length value with an optional unit.
   *
   ...
   * @return {@code true} when the syntax is {@code rect(...)} with four valid parts
   */
   ...
   * single or double quotes are removed before trimming again. Empty results are dropped. A {@code
   * null} input array is returned as {@code null} to preserve the caller contract.
   *
   ...
   * @return a newly allocated array containing only non-empty cleaned elements, in the original
   *     order
   */
