...
public interface FilterCallback {
   ...
   * a relative form according to policy. If the URI cannot be made safe enough for inclusion, the
   * method returns {@code null}. Implementations may also throw an exception to signal a hard
   * validation failure.
   *
   ...
   * relative references to be converted to absolute ones. It can also mark a URI as inline. That
   * allows an implementation to treat it as a candidate for prefetching or more permissive
   * policies. Returning {@code null} indicates the URI should be removed.
   *
   * @param uri The URI string to evaluate. Maybe absolute or relative; normalization is
   ...
   *     applied only when {@code uri} lacks a host; must include the scheme and, optionally, port.
  ...
  /**
   * Processes an individual HTML tag and optionally returns replacement serialization.
   *
