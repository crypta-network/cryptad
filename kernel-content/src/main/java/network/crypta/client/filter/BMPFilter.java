 ...
 *   <li>Validates the bit depth and supported compression codes (0–3).
...
public class BMPFilter implements ContentDataFilter {
   ...
   *
   * <p>The method attempts to read four bytes and interpret them in little-endian order (the least
   ...
   *     encoded in little-endian order; must not be {@code null}.
   * @return the decoded 16-bit value (as a positive {@code int} in the range 0–65,535) assembled
   *     from two bytes in little-endian order.
   * @throws IOException if an I/O error occurs while reading or when the end of the stream is
   *     reached before the value is complete.
   */
  ...
  public void readFilter(
    ...
    LegacyFileSupport.copy(dis, output, -1);
    output.flush();
  }
