 ...
 * {@link #produceEvent(ClientEvent, ClientEventDispatchContext)} whenever a notable occurrence
 * should be published. The dispatch strategy here is intentionally simple: the producer takes a
 * snapshot of the current listeners under synchronization, then iterates, and invokes each listener
 * once for the provided event.
 *
 ...
 * offloaded via the provided {@code ClientEventDispatchContext}.
 *
 * <ul>
...
public final class SimpleEventProducer implements ClientEventProducer, Serializable {
   ...
   * #produceEvent(ClientEvent, ClientEventDispatchContext)}.
   */
   ...
   * @param context execution context associated with the event; may be {@code null} when no
   *     dispatch context is available to the caller
   */
  @Override
  public void produceEvent(ClientEvent ce, ClientEventDispatchContext context) {
    ...
    for (ClientEventListener cel : list) {
      try {
        ClientEventCompatibility.dispatchToListener(cel, ce, context);
      } catch (Exception ue) {
