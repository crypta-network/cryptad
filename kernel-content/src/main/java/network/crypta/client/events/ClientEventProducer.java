 ...
 * updates, such as expected sizes, MIME types, or compression status, and allow external components
 ...
 * assume a particular threading model. To avoid reentrancy surprises and UI stalls, producers
 * should strive to dispatch quickly, and listeners should avoid blocking. A common pattern is to
 * pass a {@code ClientEventDispatchContext} alongside each event so listeners can schedule heavier
 * work on background persistence queues rather than executing it inline.
 *
 * <ul>
...
public interface ClientEventProducer extends Serializable {
  /**
   * Dispatches through the producer while preserving compatibility with legacy producers that still
   * implement the old {@code ClientContext}-based signature.
   *
   * @param producer target producer
   * @param ce event instance to raise
   * @param context dispatch context associated with the event
   */
  static void dispatchEvent(
      ClientEventProducer producer, ClientEvent ce, ClientEventDispatchContext context) {
    ClientEventCompatibility.dispatchToProducer(producer, ce, context);
  }
   ...
   * ClientEventDispatchContext} to queue the work rather than performing it inline.
   *
   * <pre>{@code
   ...
   *     queue follow-up work and avoid blocking inside listener callbacks
   */
  void produceEvent(ClientEvent ce, ClientEventDispatchContext context);

  /**
   * Adds a listener that will receive all later events produced by this instance.
   *
   ...
   *
   * @param cel the listener to remove; non-null; the exact matching semantics are
   *     implementation-dependent (reference equality is common)
