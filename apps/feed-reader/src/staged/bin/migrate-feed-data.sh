...
if [ "$namespace" != "ui-state" ] || [ "$from_schema" != "1" ] || [ "$to_schema" != "2" ]; then
  printf 'Feed Reader migration precondition failed\n' >&2
  exit 2
fi
if [ ! -r "$input_payload" ] || [ -z "$output_payload" ]; then
  printf 'Feed Reader migration payload channel unavailable\n' >&2
  exit 2
fi
...
case "$mode" in
  dry-run|apply)
    ...
    printf 'Feed Reader app-data migration %s validated\n' "$mode"
    ;;
  *)
    printf 'Unsupported Feed Reader migration mode\n' >&2
    exit 2
    ;;
esac
