...
public class SplitFileInserterStorageTest {
    ...
    public void testSmallSplitfileNoLastBlock() throws Exception {
        SplitFileInserterStorage storage =
            createSplitFileInserterStorage(data, size, cb, false, baseContext.clone(), cryptoAlgorithm,
                                           cryptoKey, hashes, random, memoryLimitedJobRunner, keys);
    ...
    public void testSmallSplitfileWithLastBlock() throws Exception {
        ...
        assertProperSegmentStateAndGet(storage);
        assertArrayEquals(storage.readSegmentDataBlock(0, 0),
    ...
    public void testSmallSplitfileHasKeys() throws Exception {
        ...
        SplitFileInserterStorage storage =
            createSplitFileInserterStorage(data, size, cb, false, context, cryptoAlgorithm, cryptoKey,
                                           hashes, random, memoryLimitedJobRunner, keys);
        assertProperSegmentStateAndGet(storage);
        cb.waitForHasKeys();
        for (int i = 0; i < storage.segments[0].dataBlockCount + storage.segments[0].checkBlockCount +
    ...
    public void testCancel() throws Exception {
        ...
        SplitFileInserterStorage storage =
            new SplitFileInserterStorage(data, size, cb, null, new ClientMetadata(), false, null,
                                         ...
                                         null, hashes, smallBucketFactory, checker, random,
    ...
    public void testPersistentSmallSplitfileNoLastBlockCompletion() throws Exception {
        ...
        HashResult[] hashes = getHashes(data);
        SplitFileInserterStorage storage =
            createSplitFileInserterStorage(data, size, cb, true, baseContext.clone(), cryptoAlgorithm,
                                           cryptoKey, hashes, random, memoryLimitedJobRunner, keys);
        assertProperSegmentStateAndGet(storage);
    ...
    public void testPersistentSmallSplitfileNoLastBlockFailAfterResume() throws Exception {
        ...
        context.maxInsertRetries = 2;
        SplitFileInserterStorage storage =
            createSplitFileInserterStorage(data, size, cb, true, context, cryptoAlgorithm, cryptoKey,
                                           hashes, random, memoryLimitedJobRunner, keys);
        assertProperSegmentStateAndGet(storage);
        SplitFileInserterStorage resumed = null;
    ...
    private LockableRandomAccessBuffer generateData(
        ...
        LockableRandomAccessBufferFactory factory)
        throws IOException {
    ...
    private void testRoundTripSimpleRandom(long size, CompatibilityMode cmode) throws Exception {
        ...
        SplitFileInserterStorage storage =
            createSplitFileInserterStorage(data, size, cb, false, context, cryptoAlgorithm,
                                           old ? null : cryptoKey, hashes, r, memoryLimitedJobRunner,
                                           keys);
        assertEquals(Status.ENCODED, storage.getStatus());
        ...
        FetchContext fctx =
            HighLevelSimpleClientImpl.makeDefaultFetchContext(size * 2, size * 2, smallBucketFactory,
                                                              new SimpleEventProducer());
    ...
    private void testResumeCrossSegment(long size) throws Exception {
        ...
        for (int i = 0; i < totalBlocks; i++) {
            ...
            chosen.segment.onInsertedBlock(chosen.blockNumber,
                                           chosen.segment.encodeBlock(chosen.blockNumber)
                                                         .getClientKey());
        }
        cb.waitForSucceededInsert();
        assertEquals(Status.SUCCEEDED, resumed.getStatus());
    ...
    private void testRepeatedEncodeAfterShutdownCrossSegment(long size) throws Exception {
        ...
        SplitFileInserterStorage storage =
            new SplitFileInserterStorage(data, size, cb, null, new ClientMetadata(), false, null,
                                         smallRAFFactory, true, baseContext.clone(), cryptoAlgorithm,
                                         cryptoKey, null, hashes, smallBucketFactory, checker, random,
                                         memoryLimitedJobRunner, jobRunner, ticker, keys, false, 0, 0, 0,
                                         0);
        executor.waitForIdle();
        // Has not encoded anything.
    ...
    private void testRoundTripCrossSegmentRandom(long size) throws Exception {
        ...
        SplitFileInserterStorage storage =
            createSplitFileInserterStorage(data, size, cb, false, context, cryptoAlgorithm, cryptoKey,
                                           hashes, r, memoryLimitedJobRunner, keys);
        // Encoded. Now try to decode it ...
        cb.waitForHasKeys();
        Metadata metadata = storage.encodeMetadata();
        assertEquals(Status.ENCODED, storage.getStatus());
        ...
        for (int i = 0; i < segments; i++) {
            ...
            assertArrayEquals(storage.crossSegments[i].getSegmentNumbers(),
                              fetcherStorage.crossSegments[i].getSegmentNumbers());
            assertArrayEquals(storage.crossSegments[i].getBlockNumbers(),
    ...
    private boolean addRandomBlock(
        SplitFileInserterStorage storage,
        SplitFileFetcherStorage fetcherStorage, Random random)
        throws IOException {
        ...
        for (int i = 0; i < segCount; i++) {
            while (true) {
                ...
                while (true) {
                    ...
                    boolean success =
                        segment.onGotKey(block.getClientKey().getNodeCHK(), block.getBlock());
                    assertTrue(success);
                    return true;
                }
            }
    ...
    private SplitFileInserterStorage createSplitFileInserterStorage(
        SplitFileInserterStorage storage, LockableRandomAccessBuffer data, MyCallback cb,
        MemoryLimitedJobRunner memoryLimitedJobRunner, MyKeysFetchingLocally keys) throws Exception {
        return new SplitFileInserterStorage(storage.getRAF(), data, cb, random, memoryLimitedJobRunner,
                                            jobRunner, ticker, keys, fg, persistentFileTracker, null);
    }

    private SplitFileInserterStorage createSplitFileInserterStorage(
        LockableRandomAccessBuffer data, long size, MyCallback cb, boolean persistent,
        InsertContext baseContext, byte cryptoAlgorithm, byte[] cryptoKey, HashResult[] hashes, Random r,
        MemoryLimitedJobRunner memoryLimitedJobRunner, KeysFetchingLocally keys) throws Exception {
        SplitFileInserterStorage storage =
            new SplitFileInserterStorage(data, size, cb, null, new ClientMetadata(), false, null,
                                         smallRAFFactory, persistent, baseContext, cryptoAlgorithm,
                                         cryptoKey, null, hashes, smallBucketFactory, checker, r,
                                         memoryLimitedJobRunner, jobRunner, ticker, keys, false, 0, 0, 0,
                                         0);
        storage.start();
        cb.waitForFinishedEncode();
    ...
    private SplitFileFetcherStorage createFetcherStorage(
        Metadata m1, FetchCallbackForTestingSplitFileInserter fcb,
        ArrayList<COMPRESSOR_TYPE> decompressors, short cmode, FetchContext fctx, RandomSource r)
        throws FetchException, MetadataParseException, IOException {
        return new SplitFileFetcherStorage(m1, fcb, decompressors, new ClientMetadata(), false, cmode,
                                           fctx, false, salt, URI, URI, true, new byte[0], r,
                                           smallBucketFactory, smallRAFFactory, jobRunner, ticker,
                                           memoryLimitedJobRunner, checker, false, null, null, keys);
    }
    ...
    private class FetchCallbackForTestingSplitFileInserter implements SplitFileFetcherStorageCallback {
        ...
        public void onResume(
            int succeededBlocks, int failedBlocks, ClientMetadata mimeType,
            long finalSize) {
            // Ignore.
        }
