...
public class SimpleFieldSetTest {
    ...
    public void testSimpleFieldSetPutAndGet_MultiLevel() {
        ...
        String[][] methodPairsArray_MultiLevel =
            {{"A.A.A.A", "aa"}, {"A.B.A", "ab"}, {"A.C.Cc", "ac"}, {"A.D.F", "ad"}, {"A.E.G", "ae"},
                {"A.F.J.II.UI.BOO", "af"}};
    ...
    /**
     * Tests subset(String) method putting two levels keys and fetching it through subset() method on the
     * first level and then get() on the second
     */
    @Test
    public void testSimpleFieldSetSubset_String() {
        SimpleFieldSet methodSFS = new SimpleFieldSet(true);
        String[][] methodPairsArray_MultiLevel =
            {{"A", "A", "aa"}, {"A", "B", "ab"}, {"A", "C", "ac"}, {"A", "D", "ad"}, {"A", "E", "ae"},
                {"A", "F", "af"}};
        //putting values
		for (int i = 0; i < methodPairsArray_MultiLevel.length; i++) {
			methodSFS.putSingle(methodPairsArray_MultiLevel[i][0] + SimpleFieldSet.MULTI_LEVEL_CHAR +
								methodPairsArray_MultiLevel[i][1], methodPairsArray_MultiLevel[i][2]);
		}
        //getting subsets and then values
		for (int i = 0; i < methodPairsArray_MultiLevel.length; i++) {
			assertEquals(methodSFS.subset(methodPairsArray_MultiLevel[i][0])
     ...
     * (returned if the key is not found) is set to "false" and the real value is always set to "true",
     * so we are sure if it finds the right value or not (and does not use the default).
     */
    @Test
     ...
     * value (returned if the key is not found) is set to a not present int value, so we are sure if it
     * finds the right value or not (and does not use the default).
     */
    @Test
    ...
    /**
     * Tests put(String,long) and [getLong(String),getLong(String,long)] methods consistency. The default
     * value (returned if the key is not found) is set to a not present long value, so we are sure if it
     * finds the right value or not (and does not use the default).
     */
    @Test
    ...
    /**
     * Tests put(String,char) and [getChar(String),getChar(String,char)] methods consistency. The default
     * value (returned if the key is not found) is set to a not present char value, so we are sure if it
     * finds the right value or not (and does not use the default).
     */
    @Test
    ...
    /**
     * Tests put(String,double) and [getDouble(String)|getDouble(String,double)] methods consistency. The
     * default value (returned if the key is not found) is set to a not present double value, so we are
     * sure if it finds the right value or not (and does not use the default).
     */
    @Test
    ...
    /**
     * Tests SimpleFieldSet(BufferedReader,boolean,boolean) constructor, with simple and border cases of
     * the canonical form.
     */
    @Test
    ...
    private String sfsReadyString(String[][] aStringPairsArray) {
		...
		for (int i = 0; i < aStringPairsArray.length; i++) {
			methodStringToReturn.append(aStringPairsArray[i][0]).append(KEY_VALUE_SEPARATOR)
								.append(aStringPairsArray[i][1]).append('\n');
		}
    ...
    /**
     * Searches for a key in a given String[][] array. We consider that keys are stored in String[x][0]
     *
     * @param aStringPairsArray
    ...
    private static final String[][] SAMPLE_STRING_PAIRS = {
        //directSubset
        {"foo", "bar"}, {"foo.bar", "foobar"}, {"foo.bar.foo", "foobar"}, {"foo.bar.boo.far", "foobar"},
        {"foo2", "foobar.fooboo.foofar.foofoo"}, {"foo3", KEY_VALUE_SEPARATOR + "bar"}};
