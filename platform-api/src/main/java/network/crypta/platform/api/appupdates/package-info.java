 ...
 * candidate detection, policy, pending staging, apply history, and rollback summaries out of the
 * transport router while reusing the existing catalog and AppHost primitives.
 *
 * <p>The package is intentionally conservative. The default policy is manual, catalog checks only
 * record candidates, and automatic apply is limited to eligible reviewed updates for apps that are
 * already stopped. Staged update summaries contain safe identifiers, versions, digest metadata,
 * compatibility summaries, review state, and permission deltas. They do not expose staging
 * directories, rollback directories, app tokens, browser-session tokens, signing keys, request
 * bodies, or private catalog URIs.
 *
