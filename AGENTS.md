...
- **$cryptad-core-updater** — Understand and modify the package-based CoreUpdater update system and Stable 1.0 support-lifecycle subscriber: endpoints, descriptor formats, persistence, UI wiring, and platform behaviors.
...
- **$cryptad-platform-apps** — Work on Platform API v1/contract, AppHost runtime/rollback, signed app bundles/catalogs, catalog operations and mirrors, trusted app-review receipts, production security response, app-update and Stable 1.0 lifecycle/deprecation governance, durable app data, content subscriptions, app-service grants, app-owned static UI, the browser SDK, the app UI design system/linter, app permissions/audit, sandbox providers, operator recovery surfaces, and legacy admin retirement routing.
- **$cryptad-interop-performance-gates** — Maintain Hyphanet interop, performance regression, release-certification evidence gates, production beta, Stable 1.0 RC/GA, maintenance/security-hotfix publication, lifecycle governance, and related CI and release-readiness docs.
...
- **App platform:** For Platform API/contract, AppHost runtime/rollback, app catalogs/bundles, catalog operations and mirrors, trusted app-review receipts, production security response runbooks, app-update lifecycle, durable app data, content subscriptions, app-service grants, app-owned UI routes, the browser SDK, app UI design-system/lint behavior, app permission/audit, sandbox providers, operator recovery surfaces, or legacy admin retirement routing, load `$cryptad-platform-apps`.
- **Interop/performance/certification gates:** For `tools/interop`, `tools/perf`, `tools/release-certification`, production beta release/go-no-go tooling, related CI jobs, or release-gate evidence, load `$cryptad-interop-performance-gates`.
