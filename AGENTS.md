...
- **$cryptad-platform-apps** — Work on Platform API v1/contract, AppHost runtime/rollback, signed app bundles/catalogs, trusted app-review receipts, production security response, app-update lifecycle, durable app data, content subscriptions, app-service grants, app-owned static UI, the browser SDK, the app UI design system/linter, app permissions/audit, sandbox providers, and legacy admin retirement routing.
...
- **App platform:** For Platform API/contract, AppHost runtime/rollback, app catalogs/bundles, trusted app-review receipts, production security response runbooks, app-update lifecycle, durable app data, content subscriptions, app-service grants, app-owned UI routes, the browser SDK, app UI design-system/lint behavior, app permission/audit, sandbox providers, or legacy admin retirement routing, load `$cryptad-platform-apps`.
