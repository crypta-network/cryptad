...
- **$cryptad-platform-apps** — Work on Platform API v1/contract, AppHost runtime/rollback, signed app bundles/catalogs, catalog operations and mirrors, trusted app-review receipts, production security response, app-update lifecycle, durable app data, content subscriptions, app-service grants, app-owned static UI, the browser SDK, the app UI design system/linter, app permissions/audit, sandbox providers, operator recovery surfaces, and legacy admin retirement routing.
- **$cryptad-interop-performance-gates** — Maintain Hyphanet interop, performance regression, release-certification evidence gates, the production beta release pipeline, and the production beta go/no-go dashboard under `tools/interop`, `tools/perf`, `tools/release-certification`, CI, and release-readiness docs.
