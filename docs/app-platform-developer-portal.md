...
PlatformApiContract.CURRENT_CONTRACT_VERSION = 24
```

In docs, manifests, and catalog descriptors:

- `/api/v1` identifies the transport route family.
- `contractVersion=24` identifies the current app-facing Platform API compatibility contract
  snapshot. Version 24 adds named stable-baseline registry metadata. The redacted Stable 1.0
  build-support lifecycle route is operator-only and does not advance this app contract version.
