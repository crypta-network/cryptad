...
| --- | --- |
| `operator-beta.dashboard` | Shows one generated-at snapshot with overall status, summary counts, catalogs, apps, subscriptions, Trust Graph Preview, app-service grants, legacy-admin usage, diagnostics availability, warnings, and safe recovery actions. |
...
new sandbox provider, new update policy, or any FNP/FCP/wire protocol change.
