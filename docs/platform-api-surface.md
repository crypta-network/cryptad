...
| Trust graph | Contract v7 adds the local Trust Graph Preview route family. `GET /api/v1/trust-graph/status`, `anchors`, `subjects`, `statements`, and `score` require `trust.read`; `POST /api/v1/trust-graph/import`, `POST /api/v1/trust-graph/anchors`, and `DELETE /api/v1/trust-graph/anchors/{fingerprint}` require `trust.write`. The service imports bounded `crypta.trust.statement.v1` documents, manages local anchors, verifies AppVault preview signatures when issuer public keys are present, and scores direct anchor evidence with a deterministic confidence-weighted average. Unverified imports are retained as non-contributing evidence. It is not full WoT, old plugin compatibility, moderation, background crawling, or a protocol change. Audit and evidence must not include raw trust document bodies from real users, raw request bodies, signatures, private identity material, tokens, form passwords, or local paths. |
...
| App vault | Secret and identity vault routes use `vault.secrets.read`, `vault.secrets.write`, `vault.identities.read`, `vault.identities.create`, `vault.identities.use`, and `vault.identities.manage` when the vault endpoint family is wired. `POST /api/v1/app-vault/identities` creates app-owned identities for authorized app process or browser principals without returning private material. `POST /api/v1/app-vault/identities/{identityId}/profile-document` uses an identity the app can see and use to create a profile document without exporting the private key. `POST /api/v1/app-vault/identities/{identityId}/trust-statement` requires `trust.write`, `vault.identities.read`, and `vault.identities.use` and signs only the bounded trust statement payload. Vault responses and audit entries must remain token-free, path-free, and free of raw secret values, identity private material, request bodies, private keys, and raw signatures. See [app-secret-and-identity-vault.md](app-secret-and-identity-vault.md). |
