...
| Updates | `GET /api/v1/updates/core` reports core-updater availability and includes the redacted local Stable 1.0 support-lifecycle snapshot. Contract version 24 also exposes that snapshot directly at `GET /api/v1/updates/support-lifecycle`. Unknown and stale descriptor states remain explicit and never imply support. `POST /api/v1/updates/core/download` triggers the current package download flow only when the existing updater reports that it can honor the action. |
