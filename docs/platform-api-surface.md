...
| Updates | `GET /api/v1/updates/core` reports only core-updater availability and current download-action readiness. It remains app-readable for principals with `updates.read` and does not embed lifecycle state. The separate host/operator-only `GET /api/v1/updates/support-lifecycle` route does not advance the app-facing contract version; the Web Shell fetches it independently and renders the redacted local Stable 1.0 support snapshot. Unknown and stale descriptor states remain explicit and never imply support. `POST /api/v1/updates/core/download` triggers the current package download flow only when the existing updater reports that it can honor the action. |
