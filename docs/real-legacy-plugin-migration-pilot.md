...
Preserve source and target backups separately. App-data private backup envelopes currently declare `encryption.mode=none`: they are not encrypted. Use the owning app's private export/restore controls for its data and the existing operator backup surface when a complete app backup is needed. Generic restore may replace data and is not an additive migration shortcut; consult [backup/restore portability](app-data-backup-restore-portability.md).
