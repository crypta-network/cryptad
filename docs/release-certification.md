...
| `catalog.operations-and-mirrors` | App-platform smoke summary. | Catalog sources support a primary source plus mirrors, mirror transport fallback preserves signed verification, stale/downgrade replacement is blocked, verified revision history and explicit rollback re-verification are present, key-rotation status and emergency advisory refresh are visible, Platform API/Web Shell operations are wired, and evidence is redacted without requiring live mirrors. |
...
| `third-party-developer.*` | App-platform smoke summary. | Third-party developer beta docs, `hello-stable` template, checked-in stable-only sample app, public submission checklist, compatibility support-window policy, feedback templates, plugin migration path, sample app submission flow, operator-only negative path, and redaction checks are present. The group expands to `third-party-developer.beta-program`, `third-party-developer.docs`, `third-party-developer.template`, `third-party-developer.sample-app-flow`, `third-party-developer.submission-checklist`, `third-party-developer.compatibility-window`, `third-party-developer.feedback-workflow`, `third-party-developer.plugin-author-migration`, and `third-party-developer.redaction`. |
...
| `platform-api.deprecation-window-policy` | App-platform smoke summary. | Stable deprecation and scheduled-removal windows meet the minimum contract-version runway and stable removals require a future baseline. |
